...
%% Use this to correct a situation where a sample
%% which is the last in a sample-group just does
%% fill a line, resulting in too much of a vertical
%% gap (apparently the lack of horizontal space
%% on the sample's line causes an excess of
%% vertical space, as if a new line is typeset
%% to contain some post-sample space which
%% actually doesn't *need* to be there.
\newcommand{\swlEliminateGap}{\vspace{-1em}}
...
- i=id p=page s=ud source, section number c=classification^^J%
  l=label x=latex label t=text r=pre/zPage o=post a=archival ^^J%
.^^J%
^^J%
...
%else
{\ifthenelse{\equal{\detokenize{#1}}{\detokenize{(*)}}}
{\raisebox{2pt}{{\small\textbf{*}\hspace*{2pt}}}}
%else
{#1}}}
