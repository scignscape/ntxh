...
Mosaic_Menubar::Mosaic_Menubar(QMainWindow* qmw) : QMenuBar()
{
...
{
 QDir qd (DEFAULT_SCREENSHOT_FOLDER);
...
QWidgetAction* Mosaic_Menubar::add_text_separator_before(QAction* after, QString text, QWidget* parent)
{
 if(!parent)
   parent = this;
 QLabel* lbl = new QLabel(text, parent);
 ...
 result->setDefaultWidget(lbl);
 result->setProperty("menu-separator", true);
 insertAction(after, result);
 return result;
}

QWidgetAction* Mosaic_Menubar::add_text_separator_before(QMenu* after, QString text, QWidget* parent)
{
 return add_text_separator_before(after->menuAction(), text, parent);
}


QPair<QWidgetAction*,QWidgetAction*> Mosaic_Menubar::add_text_separator_around(QAction* before, QAction* after, QStringList text, QWidget *parent)
{
 return {
   add_text_separator_before(before, text.value(0), parent),
   add_text_separator_before(after, text.value(1), parent)};
}

QPair<QWidgetAction*,QWidgetAction*> Mosaic_Menubar::add_text_separator_around(QMenu* before, QMenu* after, QStringList text, QWidget *parent)
{
 return add_text_separator_around(before->menuAction(), after->menuAction(), text, parent);
}
