...
QPoint MapGraphicsObject::map_to_view(const QPointF &pos)
{
 return containing_view_->map_ll_to_view(pos);
}

QPoint MapGraphicsObject::map_pos_to_view()
{
 return map_to_view(pos());
}


QPointF MapGraphicsObject::map_to_scene(const QPointF &pos)
{
 return containing_view_->map_ll_to_scene(pos);
}

QPointF MapGraphicsObject::map_pos_to_scene()
{
 return map_to_scene(pos());
}
...
void MapGraphicsObject::contextMenuEvent(QGraphicsSceneContextMenuEvent* event)
{
 if(this->isSelected())
 {
  containing_view_->handle_context_menu(event, this);
 }
 else
  event->ignore();
}
