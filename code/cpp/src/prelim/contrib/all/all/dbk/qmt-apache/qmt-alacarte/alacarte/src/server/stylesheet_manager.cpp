...
// std::string ss;
// for(auto const& pair : parsedStylesheets)
// {
//  fs::path fsp = pair.first;
//  ss = fsp.string();
// }
...
void StylesheetManager::onNewStylesheet(const fs::path& stylesheet_path,
  const fs::path& stylesheet_folder)
{
...
//?		stylesheet = Stylesheet::Load(stylesheetFolder / filename, manager->getGeodata(), timeout);
  stylesheet = Stylesheet::Load(stylesheet_folder / filename, manager->getGeodata(), timeout);
...
// //  qmt ...
void StylesheetManager::check_parse_new_stylesheet(const string& stylesheet_path,
  const string& stylesheet_folder_path)
{
 if(hasStylesheet(stylesheet_path))
   return;

 onNewStylesheet(stylesheet_path, stylesheet_folder_path);
