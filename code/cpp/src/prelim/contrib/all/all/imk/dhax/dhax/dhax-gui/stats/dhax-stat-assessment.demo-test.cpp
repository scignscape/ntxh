...
void (*_make_run_contours())(DHAX_Stat_Assessment&)
{
 return [](DHAX_Stat_Assessment& stat)
 {
  ...
  cv::Mat threshold_full, threshold_1c;
  ...
  std::vector<std::vector<cv::Point>> contours_full, contours_1c;
  std::vector<cv::Vec4i> hierarchy_full, hierarchy_1c;
  ...
  auto it_full = std::max_element(contours_full.begin(), contours_full.end(), [](std::vector<cv::Point>& lhs,
    std::vector<cv::Point>& rhs)
  {
   return lhs.size() < rhs.size();
  });

  auto it_1c = std::max_element(contours_1c.begin(), contours_1c.end(), [](std::vector<cv::Point>& lhs,
    std::vector<cv::Point>& rhs)
  {
   return lhs.size() < rhs.size();
  });
  ...
  qDebug() << "max = " << max_1c.size();

  std::vector<std::vector<cv::Point>> larger_contours_full, larger_contours_1c;

  std::vector<std::vector<cv::Point>> largest_contours_full, largest_contours_1c;
  ...
  {
//   qDebug() << "size = " << contours_1c[i].size();
...
//#endif
  ...
  cv::Mat image_full, out_full, image_1c, out_1c, out_full_1c;
  ...
  cv::drawKeypoints(stat.one_channel_display_image(),
     keypoints_1c, out_1c, cv::Scalar(0, 255, 0));
  cv::imwrite(stat.get_1c_out_path().toStdString(), out_1c);

  cv::drawKeypoints(stat.full_image(),
     keypoints_1c, out_full_1c, cv::Scalar(0, 255, 0));
  cv::imwrite(stat.get_full_1c_out_path().toStdString(), out_full_1c);
...
#define setup(ALGORITHM_NAME) \
 ...
 ALGORITHM_NAME##_stat->set_one_channel_image_path(one_channel); \
 ALGORITHM_NAME##_stat->set_one_channel_display_image_path(one_channel_display); \
