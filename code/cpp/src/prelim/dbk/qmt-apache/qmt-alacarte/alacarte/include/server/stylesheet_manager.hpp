...
public:
 ...
 void check_parse_new_stylesheet(const string& stylesheet_path,
   const string& stylesheet_folder_path);

private:

 void onNewStylesheet(const fs::path& stylesheet_path, const fs::path&
   stylesheet_folder);
 ...
 {
  onNewStylesheet(stylesheet_path, stylesheetFolder);
 }
