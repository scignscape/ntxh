...
// // qmt
shared_ptr<TileIdentifier> TileIdentifier::Create(int zoom, int x, int y,
  const string& format, const string& stylesheet_path_name,
  const string& stylesheet_folder_path,
  ...
  const shared_ptr<Configuration>& config)
{
 ...
 if(stylesheet_folder_path.length() > 0)
 {
  // // try parsing the sheet if we haven't done so
  stylesheetManager->check_parse_new_stylesheet(stylesheet_path_name, stylesheet_folder_path);

 }


 string styleSheetpath = stylesheet_path_name;
