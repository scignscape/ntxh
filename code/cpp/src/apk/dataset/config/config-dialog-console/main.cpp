...
int main(int argc, char **argv)
{
 ...
 dlg.set_reset_callback([]()
 {
  ...
  {
   DEFINES_SRC_FOLDER "/uni-custom_defines.h",
   CHOICES_PRI_FOLDER "/uni-custom_choices.pri",
   UNIBUILD_PRI_FOLDER "/build-custom.pro",
   CUSTOM_LIBS_PRI_FOLDER "/_xpdf.pri",
   CUSTOM_LIBS_PRI_FOLDER "/_kph.pri",
   CUSTOM_LIBS_PRI_FOLDER "/_dgdb.pri",
  }, ".reset");
 });
 ...
 dlg.set_proceed_callback([&dlg](QString qs)
 {
  ...
  {
   QString result;
   QString f = acm.insert_to_defines(DEFINES_SRC_FOLDER "/uni-custom_defines.h", result);
   save_file(f, result);
  }

  {
   QString result;
   QString f = acm.insert_to_choices(CHOICES_PRI_FOLDER "/uni-custom_choices.pri", result);
   save_file(f, result);
  }
...

  {
   ...
   QMap<QString, QString> files {{
     ...
     { "dgdb", CUSTOM_LIBS_PRI_FOLDER "/_dgdb.pri" }
    }};
