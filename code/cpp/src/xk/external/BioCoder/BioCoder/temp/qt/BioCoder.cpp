...
void Time::display_time()
{
 ...
 if ((value == OVERNIGHT)&&(unit == "hrs"))
  ...
  qts_write(qts, "<b>immediately</b>");
 else if (value == FOREVER)
  qts_write(qts, "<b><font color=#357EC7>forever</font></b>(until taken out from the thermocycler)");
 else if (value == XVAL)
  qts_write(qts, "<b><font color=#357EC7>X</font></b>");
 else 
...
void Minimum_time::display_time()
{
 ...
 qts_write(qts, "at least <b><font color=#357EC7>%g %s</font></b>", value, unit);
 if((option_no == 2) && (options_flag == 1))
  max_time_value = value * mul;
 else if(option_no > 2)
 {
...
{ 
 ...
 qts_write(qts, "at most <b><font color=#357EC7>%g %s</font></b>", value, unit);
 if((option_no == 2) && (options_flag == 1))
  max_time_value = value * mul;
 else if(option_no > 2)
 {
...
{ 
 ...
 qts_write(qts, "~<b><font color=#357EC7>%g %s</font></b>", value, unit);
 if((option_no == 2) && (options_flag == 1))
  max_time_value = value * mul;
 else if(option_no > 2)
 {
...
void Symbolic_time:: display_time()
{
 Time* time1;
 if (s1.value == DEFAULT)
  qts_write(qts,"<b><font color=#357EC7>%s secs</font></b>", s1.symbol); 
 else 
 {
...
void Minimum_speed::display_speed()
{
 ...
 qts_write(qts, "a speed of at least <font color=#357EC7>%g %s</font>", value, unit);
};
...
void Maximum_speed::display_speed()
{
 ...
 qts_write(qts, "a speed of at most <font color=#357EC7>%g %s</font>", value, unit); 
};
...
void Approx_speed::display_speed()
{
 ...
 qts_write(qts, "a speed of ~<font color=#357EC7>%g %s</font>", value, unit); 
};
...
Volume::Volume(float x, enum vol_unit unit1)
{
 ...
 switch(unit_choice)
 {
  case UL: unit = "\B5l";mul = MICRO;break;
  case ML: unit = "ml";mul = MILLI;break;
  case L: unit = "l";mul = LITRE;break;
  default : unit = "\B5l";mul = MICRO;break;
 }
}
...
void Symbolic_vol:: display_vol()
{
 Volume* vol1;
 if (s1.value == DEFAULT)
  qts_write(qts,"<b><font color=#357EC7>%s \B5l</font></b>", s1.symbol); 
 else 
 {
...
{
 qts_write(qts, "<b><font color=#357EC7>%g - %g %s</font></b>", value1, value, unit);
};
...
void start_protocol(char* name)
{
 ...
 fp = fopen(temp1, "w");
 qts_write(qts, "<h1 style=\"font-size = 25px;\">%s</h1>", name);
}

void first_step()
{
 first = 1;
 qts_write(qts, "</ul><h2>Steps:</h2><ol>");
 qts_write(qts, "<p><li>");
}
...
void first_step(char* step_name)
{
 first = 1;
 qts_write(qts, "</ul><h2>Steps:</h2><ol>");
 qts_write(qts, "<p><li><b><font size=3>%s</font></b><br>", step_name);
}
...
void next_step(char* step_name)
{
 ...
 if (sub_step_flag == 1)
 {
  qts_write(qts,"</li></p></ol>");
  sub_step_flag = 0;
 }
 qts_write(qts, "</li></p><p><li><b><font size=3>%s</font></b><br>", step_name);
}
...
void optional_step()
{
 ...
 qts_write(qts, "</li></p><p><b><font size=3>(Optional)</font></b><br>");
}
...
void optional_step(char* name)
{
 ...
 qts_write(qts, "</li></p><p><b><font size=3>%s (Optional)</font></b><br>", name);
}
...
void parallel_step()
{
 ...
 qts_write(qts, "</li></p><p><li><b>Meanwhile:</b><br>");
}

void first_sub_step()
{
 first = 0;
 qts_write(qts, "<ol type=\"a\"><p><li>");
 sub_step_flag = 1;
}
...
{
 qts_write(qts, "Repeat Step %d. <br>", step_no);
}

void first_option()
{
 options_flag = 1;
 qts_write(qts,"<p><b>Option %d: </b>", option_no);
 option_no++;
}
...
{
 options_flag = 1;
 qts_write(qts,"<p><b>Option %d: %s</b>", option_no, name);
 option_no++;
}
...
void next_option()
{
 ...
 qts_write(qts,"(or)<br><b>Option %d: </b>",option_no);
 option_no++;
}
...
void next_option(char* name)
{
 ...
 qts_write(qts,"(or)<p><b>Option %d: %s</b>",option_no, name);
 option_no++;
}
...
{
 int i;
 qts_write(qts, "<div style=\"top: 25px; margin-top: 50px; margin-left: 700px; position: absolute; z-index: 1; visibility: show;\">");
 qts_write(qts, "<h2>Equipment:</h2><ul type=\"circle\">");
 for(i=0; i<equip_no; i++)
  qts_write(qts, "<li>%s</li>", equipments[i]);
 for(i=0; i<cont_no; i++)
  qts_write(qts, "<li>%s</li>", containers[i]);
 qts_write(qts, "</ul></div>");
}
...
void usage_details()
{
 ...
 for(i=0;i<list_fluid_no;i++)
 {
  ...
  if (count1 == 0)
  {
   qts_write(qts, "<font color = red>Warning: The following reagents/materials were declared but not used in the protocol:</font><ul>");
   qts_write(qts, "<li>%s</li>", usage_list_fluids[i].original_name);
   count1++;
  }
  else
   qts_write(qts, "<li>%s</li>", usage_list_fluids[i].original_name);
 ...
 for(i=0;i<list_container_no;i++)
 {
  ...
  if (count2 == 0)
  {
   qts_write(qts, "<font color = red>Warning: The following equipment were declared but not used in the protocol:</font><ul>");
   qts_write(qts, "<li>%s</li>", usage_list_containers[i].name);
   count1++;
  }
  else
   qts_write(qts, "<li>%s</li>", usage_list_containers[i].name);
...
void timing_details()
{
 ...
 if (time_hrs == 0)
  qts_write(qts, "<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d mins</font></b></p>", time_mins);
 else if (time_hrs == 1)
  qts_write(qts, "<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d hr, %d mins</font></b></p>", time_hrs, time_mins);
 else
  qts_write(qts, "<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d hrs, %d mins</font></b></p>", time_hrs, time_mins);
...
void check_container(Container& container1)
{
 if ((first == 1) || (prev_cont == 1)||(prev_container != container1.name))
 {
  ...
  qts_write(qts, "%s", container1.name);
 }
}

void to_do(char* clarify)
{
 qts_write(qts, "<font color = \"red\"><i>%s</i></font><br>", clarify);
}

void comment(char* comm)
{
 qts_write(qts, "<font color = \"#800517\"><i>%s</i></font><br>", comm);
}
...
Fluid new_fluid(char *name)
{
 ...
 if(print_reagents == 1)
 {
  qts_write(qts, "<h2 style=\"margin-top:50px;\">Solutions/reagents:</h2><ul type=\"circle\">");
  print_reagents++;
 }
 qts_write(qts, "<li>%s</li>", name);
 result.used = 0;
...
{ 
 ...
 if(temp == ICE_COLD)
 {
  qts_write(qts, "<li>ice-cold %s</li>", name, temp);
  ...
  qts_write(qts, "<li>%s stored at room temperature</li>", name);
 else
  qts_write(qts, "<li>%s stored at %g%cC</li>", name, temp, 0x00B0);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
Fluid new_fluid(char*name, char* state)
{
 ...
 qts_write(qts, "<li> <a name=\"%s\">%s <i><br><tab><div style=\"margin-right: 600px;\">(%s)</div></i></a></li>", name, name, state);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
Fluid new_fluid (char* name, char* state, float temp)
{
 ...
 if(temp == ICE_COLD)
 {
  ...
  strcat(t1, name);
  result.new_name = t1;
  qts_write(qts, "<li> <a name=\"%s\"> %s <i><br><tab><div style=\"margin-right: 600px;\">(%s)</div></i></a></li>", result.new_name, result.new_name, state);
 }
 else if(temp >= 35)
  qts_write(qts, "<li> <a name=\"%s\">%s preheated in a water bath set at <b><font color=#357EC7>%g%cC</font></b> <i>%s</i></a></li>", name, name, temp, 0x00B0, state);
 else
  qts_write(qts, "<li> <a name=\"%s\">%s <i>%s</i> </a> at <b><font color=#357EC7>%g%cC</font></b></li>", name, name, state, temp, 0x00B0);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
{ 
 ...
 if(temp == ICE_COLD)
 {
  ...
  qts_write(qts, "<li>%s at %g%cC</li>", name, temp, 0x00B0);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
Solid new_solid(char*name, char* state)
{
 ...
 qts_write(qts, "<li> <a name=\"%s\">%s <i><br><tab>(%s)<br></i></a></li>", name, name, state);
 result.used = 0;
 usage_list_fluids[list_fluid_no] = result;
 result.usage_index = list_fluid_no;
 list_fluid_no++;
...
Solid new_solid (char* name, char*state, float temp)
{
 ...
 if(temp == ICE_COLD)
 {
  qts_write(qts, "<li> <a name=\"%s\">ice-cold %s <i><br><tab>(%s)<br></i></a></li>", name, name, state);
  t1 = (char *)calloc(strlen(t1) + strlen(name) + 10, sizeof(char));
  strcat(t1, "ice-cold ");
  strcat(t1, name);
  result.new_name = t1;
...
void measure_fluid(Fluid& fluid1, Container& container1)
{
 // graph maintenance
 {
  ...
  if (fluid1.volume == 0)
   qts_write(qts, "<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", fluid1.new_name);
  if (usage_list_fluids[fluid1.usage_index].original_name == fluid1.original_name)
   usage_list_fluids[fluid1.usage_index].used = 1;
  else 
  {
  ...
  if (first == 1)
  {
   ...
   {
    if(fluid1.state == "")
     qts_write(qts, "Measure out %s into %s.<br>", fluid1.new_name, container1.name);
    else
     qts_write(qts, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.name);
    first = 0;
    prev_cont++;
   }
   else
   {
    if(fluid1.state == "")
     qts_write(qts, "Measure out %s into %s.<br>", fluid1.new_name, container1.contents.new_name);
    else
     qts_write(qts, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.contents.new_name);
    first = 0;
    prev_cont++;
   }
  }
  ...
  {
   qts_write(qts, "Add <font color=#357EC7>%s</font> to %s.<br>", fluid1.new_name, container1.contents.new_name);
   prev_container = container1.name;
  }
...
void measure_fluid(Container& container, Container& container1)
{
 // graph maintenance
 {
  ...
  if (container.volume == 0)
   qts_write(qts, "<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", container.contents.new_name);
  ...
  if (first == 1)
  {
   ...
   if (container1.contents.new_name == "")
   {
    if(container.contents.state == "")
     qts_write(qts, "Measure out %s into %s.<br>", container.contents.new_name, container1.name);
    else
     qts_write(qts, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", container.contents.new_name, container.contents.new_name, container1.name);
    first = 0;
    prev_cont++;
   }
   else
   {
    if(container.contents.state == "")
     qts_write(qts, "Measure out %s into %s.<br>", container.contents.new_name, container1.contents.new_name);
    else
     qts_write(qts, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", container.contents.new_name, container.contents.new_name, container1.contents.new_name);
    first = 0;
    prev_cont++;
   }
  }
  ...
  {
   qts_write(qts, "Add <font color=#357EC7>%s</font> to %s.<br>", container.contents.new_name, container1.contents.new_name);
   prev_container = container1.name;
  }
...
void measure_fluid(Fluid& fluid1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Measure out ");
  volume1->display_vol();
  if(fluid1.state == "")
   qts_write(qts, " of %s.<br>", fluid1.new_name);
  else
   qts_write(qts, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", fluid1.new_name, fluid1.new_name);
...
void measure_fluid(Fluid &fluid1, Volume* volume1, Container& container1)
{
 // graph maintenance
 {
  ...
  if (first == 1)
  {
   ...
   {
    if(fluid1.state == "")
     qts_write(qts, " of <font color=#357EC7>%s</font> into %s.<br>", fluid1.new_name, container1.name);
    else
     qts_write(qts, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.name);
    first = 0;
    prev_cont++;
   }
   else
   {
    if(fluid1.state == "")
     qts_write(qts, " of <font color=#357EC7>%s</font> into %s.<br>", fluid1.new_name, container1.contents.new_name);
    else
     qts_write(qts, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.contents.new_name);
    first = 0;
    prev_cont++;
   }
  }
  ...
  else if (prev_container == container1.name)
  {
   ...
   if(fluid1.state == "")
   {
    qts_write(qts, "Add ");
    volume1->display_vol();
    qts_write(qts, " of <font color=#357EC7>%s</font>.<br>", fluid1.new_name);
   }
   else
   {
    qts_write(qts, "Add ");
...
void measure_fluid(Container& con, Volume* volume1, Container& container1)
{
 // graph maintenance
 {
  ...
  if ((con.volume == 0)|| (con.volume < (volume1->value*volume1->mul)))
   qts_write(qts, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", con.contents.new_name);
  ...
  if (first == 1)
  {
   ...
   if (container1.contents.new_name == "")
   {
    if(con.contents.state == "")
     qts_write(qts, " of <font color=#357EC7>%s</font> into %s.<br>", con.contents.new_name, container1.name);
    else
     qts_write(qts, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", con.contents.new_name, con.contents.new_name, container1.name);
    first = 0;
    prev_cont++;
   }
   else
   {
    if(con.contents.state == "")
     qts_write(qts, " of <font color=#357EC7>%s</font> into %s.<br>", con.contents.new_name, container1.contents.new_name);
    else
     qts_write(qts, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", con.contents.new_name, con.contents.new_name, container1.contents.new_name);
    first = 0;
    prev_cont++;
   }
  }
  ...
  else if (prev_container == container1.name)
  {
   ...
   if(con.contents.state == "")
   {
    qts_write(qts, "Add ");
    volume1->display_vol();
    qts_write(qts, " of <font color=#357EC7>%s</font>.<br>", con.contents.new_name);
   }
   else
   {
    qts_write(qts, "Add ");
    volume1->display_vol();
    qts_write(qts, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", con.contents.new_name, con.contents.new_name);
   }
  }
  else
  {
...
void set_temp(Container& container1, float temp)
{
 // graph maintenance
 {
  ...
  if (temp >= 35)
   qts_write(qts, "Pre-heat %s in a water bath set at <b><font color=#357EC7>65%cC</font></b>.<br>", container1.contents.new_name, 0x00B0);
  else
   qts_write(qts, "Set the temperature of %s to <b><font color=#357EC7>%g%cC</font></b>.<br>", container1.contents.new_name, temp, 0x00B0);
}
...
void measure_prop(Container& dest, Container& source, float prop)
{
 // graph maintenance
 {
  ...
  if ((source.contents.volume == 0)|| (source.contents.volume < (dest.volume * prop)))
   qts_write(qts, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", source.contents.new_name);
  ...
  if (first == 1)
  {
   ...
   {
    if (prop > 1)
     qts_write(qts, "Measure out <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font> into %s.<br>", prop, source.contents.new_name, dest.contents.new_name);
    else
     qts_write(qts, "Measure out <b><font color=#357EC7>%g</font></b> volume <font color=#357EC7>%s</font> into %s.<br>", prop, source.contents.new_name, dest.contents.new_name);
   }
   else
   {
    if (prop > 1)
     qts_write(qts, "Measure out <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", prop, source.contents.new_name, source.contents.new_name, dest.contents.new_name);
    else
     qts_write(qts, "Measure out <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", prop, source.contents.new_name, source.contents.new_name, dest.contents.new_name);
   }
  }
  else if (prev_container == dest.name)
  {
   ...
   {
    if (prop > 1)
     qts_write(qts, "Add <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", prop, source.contents.new_name, source.contents.new_name);
    else 
     qts_write(qts, "Add <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", prop, source.contents.new_name, source.contents.new_name);
   }
   else 
   {
    if (prop > 1)
     qts_write(qts, "Add  <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font>.<br>", prop, source.contents.new_name);
    else
  ...
  else
  {
   ...
   {
    if (prop > 1)
     qts_write(qts, "Add <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", prop, source.contents.new_name, source.contents.new_name, dest.name, dest.name);
    else 
     qts_write(qts, "Add <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", prop, source.contents.new_name, source.contents.new_name, dest.name);
   }
   else 
   {
    if (prop > 1)
     qts_write(qts, "Add  <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font> to %s.<br>", prop, source.contents.new_name, dest.name);
    else
...
void measure_prop(Container& dest, Fluid& source, float prop)
{
 // graph maintenance
 {
  ...
  if ((source.volume == 0)|| (source.volume < (dest.volume*prop)))
   qts_write(qts, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", source.new_name);
  ...
  if (first == 1)
  {
   ...
   {
    if (prop > 1)
     qts_write(qts, "Measure out <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font> into %s.<br>", prop, source.new_name, dest.contents.new_name);
    else
     qts_write(qts, "Measure out <b><font color=#357EC7>%g</font></b> volume <font color=#357EC7>%s</font> into %s.<br>", prop, source.new_name, dest.contents.new_name);
   }
   else
   {
    if (prop > 1)
     qts_write(qts, "Measure out <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", prop, source.new_name, source.new_name, dest.contents.new_name);
    else
     qts_write(qts, "Measure out <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", prop, source.new_name, source.new_name, dest.contents.new_name);
   }
  }
  else if (prev_container == dest.name)
  {
   ...
   {
    if (prop > 1)
     qts_write(qts, "Add <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", prop, source.new_name, source.new_name);
    else 
     qts_write(qts, "Add <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", prop, source.new_name, source.new_name);
   }
   else 
   {
    if (prop > 1)
     qts_write(qts, "Add  <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font>.<br>", prop, source.new_name);
    else
     qts_write(qts, "Add  <b><font color=#357EC7>%g</font></b> volume <font color=#357EC7>%s</font>.<br>", prop, source.new_name);
   }
  }
  else
  {
   ...
   {
    if (prop > 1)
     qts_write(qts, "Add <b><font color=#357EC7>%g</font></b> volumes <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", prop, source.new_name, source.new_name, dest.name);
    else 
     qts_write(qts, "Add <b><font color=#357EC7>%g</font></b> volume <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", prop, source.new_name, source.new_name, dest.name);
   }
   else 
   {
    if (prop > 1)
     qts_write(qts, "Add  <b><font color=#357EC7>%g</font></b> volumes <font color=#357EC7>%s</font> to %s.<br>", prop, source.new_name, dest.name);
    else
     qts_write(qts, "Add  <b><font color=#357EC7>%g</font></b> volume <font color=#357EC7>%s</font> to %s.<br>", prop, source.new_name, dest.name);
   }
  }
...
void transfer (Container &container1, Container& container2)
{
 // graph maintenance
 {
  ...
  if (container1.volume == 0)
   qts_write(qts, "<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", container1.contents.new_name);
  ...
  qts_write(qts, "Transfer %s into %s.<br>" , container1.contents.new_name, container2.name);
...
void centrifuge_pellet(Container& container1, Speed* speed1, float temp, Time* time1, Volume* expected_vol)
{
 ...
 time1->display_time();
 if(temp == RT)
  qts_write(qts, "at <b><font color=#357EC7>room temperature</font></b>, gently aspirate out the supernatant and discard it.<br>");
 else
  qts_write(qts, "at <b><font color=#357EC7>%g%cC</font></b>, gently aspirate out the supernatant and discard it.<br>", temp, 0x00B0);
 qts_write(qts, "<i>The expected volume of supernatant is ");
...
void centrifuge_phases_top(Container &container1, Speed* speed1, float temp, Time* time1, Volume* aspirate_vol, Container &container2)
{
 ...
 time1->display_time();
 if(temp == RT)
  qts_write(qts, " at <b><font color=#357EC7><b><font color=#357EC7>room temperature</font></b></font></b>");
 else
  qts_write(qts, " at <b><font color=#357EC7>%g%cC</font></b>", temp, 0x00B0);
 fprintf( fp, " and aspirate out ");
 aspirate_vol->display_vol();
 qts_write(qts, " of top layer into %s.<br>", container2.name);
...
void centrifuge(Container &container1, Speed* speed1, float temp, Time* time1)
{
 ...
 time1->display_time();
 if(temp == RT)
   qts_write(qts, " at <b><font color=#357EC7><b><font color=#357EC7>room temperature</font></b></font></b>.<br>");
 else
   qts_write(qts, " at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
 if(centrifuge_no == 1)
 {
  equipments[equip_no] = "Centrifuge";
  equip_no++;
...
{ 
 ...
 qts_write(qts, "Discard %s.<br>", container1.contents.new_name);
 container1.volume = 0;
}
...
void mix (Container &container1, enum mixing type, Time* time1)     
{
 switch(type)
 {
 case TAPPING: qts_write(qts, "Gently tap the mixture for ");time1->display_time();qts_write(qts, " .<br>"); break;
 case STIRRING: qts_write(qts, "Stir the mixture for ");time1->display_time();qts_write(qts, " .<br>"); break;
 case INVERTING: qts_write(qts, "Close the tube tightly and invert the tube ");time1->display_time();qts_write(qts, " times.<br>"); break;
 case VORTEXING: qts_write(qts, "Vortex the mixture for ");time1->display_time();qts_write(qts, " .<br>"); break;
 default: qts_write(qts, "Invalid entry.<br>");
 }
 // graph maintenance
 {
...
void mix (Container &container1, enum mixing type, int times)     
{
 ...
 {
 case INVERTING: 
  qts_write(qts, "Close the tube tightly and invert the tube <b><font color=#357EC7>%d times</font></b>.<br>", times); 
  break;
 default: 
  qts_write(qts, "Invalid entry.<br>");
 }
 // graph maintenance
 {
  Operation o = new_operation("mix");
...
void mix (Container &container1, enum mixing type, int min_times, int max_times)     
{
 ...
 {
 case INVERTING: 
  qts_write(qts, "Close the tube tightly and invert the tube <b><font color=#357EC7>%d - %d times</font></b>.<br>", min_times, max_times); break;
 default: 
  qts_write(qts, "Invalid entry.<br>");
 }
 // graph maintenance
...
void mix (Container &container1, enum mixing type)     
{
 ...
 {
 case TAPPING: 
  qts_write(qts, "Gently tap the mixture for a few secs.<br>"); 
  break;
 case STIRRING: 
  qts_write(qts, "Stir the mixture for a few secs.<br>"); 
  break;
 case INVERTING: 
  qts_write(qts, "Close the tube tightly and gently mix the contents by inverting the tube.<br>"); 
  break;
 case VORTEXING: 
  qts_write(qts, "Vortex the mixture for a few secs.<br>"); 
  break;
 case RESUSPENDING:
  qts_write(qts, "Resuspend pellet by vortexing/by shaking vigorously.<br>"); 
  break;
 case DISSOLVING:
  qts_write(qts, "Dissolve the pellet in the solution.<br>");
  break;
 case PIPETTING:
  qts_write(qts, "Mix solution by pipetting up and down several times.<br>");
  break;
 default: q
  ts_write(qts, "Invalid entry.<br>");
 }
 // graph maintenance
 {
  Operation o = new_operation("mix");
...
void mix (Container &container1, enum until event1)     
{
 ...
 qts_write(qts, "Close the tube tightly and gently mix the contents by inverting the tube until precipitate stops sticking to walls of the tube.<br>");
 if (event1 == PELLET_DISLODGES)
  qts_write(qts, "Gently mix the contents of the tube until the pellet dislodges.<br>");
 // graph maintenance
 {
  Operation o = new_operation("mix");
  create_edge_from_container_to_fluid(&container1, &o);
...
{
 qts_write(qts, "Keep %s aside for ", container1.contents.new_name);
 time1->display_time();
 qts_write(qts, ".<br>");
 // graph maintenance
 {
...
void combine_and_mix (mixing type, Time* time1, int count, Container &container1, ...)
{
 va_list ap;
 va_start(ap, container1);
 combine_helper(count, container1, ap);
...
void combine_and_mix(mixing type, int count, Container &container1, ...)
{
 va_list ap;
 va_start(ap, container1);
 combine_helper(count, container1, ap);
...
void inoculation(Container& medium, Fluid& sample1, Volume* volume1, float temp, Time* time1, int shaking_type)
{
 ...
 if(medium.volume != DEFAULT)
 {
  ...
  qts_write(qts, QString("Inoculate <font color=#357EC7>%1 %2 %3</font> with ").arg(result).arg(unit).arg(medium.contents.new_name));
 }
 else
  qts_write(qts, QString("Inoculate <font color=#357EC7>%1</font> with ")
  .arg(medium.contents.new_name));
 volume1->display_vol();
 switch(shaking_type)
 {
  case 1:
   qts_write(qts, QString(" of <font color=#357EC7>%1</font> and incubate with shaking for ").arg(sample1.new_name));
   break;
  case 2:
   qts_write(qts, QString(" of <font color=#357EC7>%1</font> and incubate without shaking for ").arg(sample1.new_name));
   break;
  default:qts_write(qts, "Invalid inputs.");
   break;
 }
 time1->display_time();
 qts_write(qts, QString(" at <b><font color=#357EC7>%1%2C</font></b>.<br>").arg(temp).arg(0x00B0)); // // degree ... 
...
void inoculation(Container& medium, Fluid& sample1, Volume* volume1, float temp, Time* time1, float cell_density, int shaking_type)
{
 ...
 float result;
 if ((sample1.volume == 0)|| (sample1.volume < (volume1->value*volume1->mul)))
   qts_write(qts, QString("<font color = red>Warning: You are measuing out more than the available volume of %1! Please make sure you have enough before carrying on with the protocol.<br></font>").arg(sample1.new_name));
 ...
 if (medium.volume != DEFAULT)
 {
  ...
  qts_write(qts, QString("Inoculate <font color=#357EC7>%1</font> with ").arg(medium.contents.new_name));
 volume1->display_vol();
 switch(shaking_type)
 {
  case 1:
 ...
 time1->display_time();
 qts_write(qts, QString(" at <b><font color=#357EC7>%1%2C</font></b>.<br>".arg(temp).arg(0x00B0));
 qts_write(qts, QString("Calibrate the spectrophotometer and make sure that the cell density of the culture has reached <b><font color=#357EC7>%1 X 10<sup>6</sup> cells/ml</font></b> before continuing.<br>").arg(cell_density));
 medium.volume = medium.volume + volume1->value * volume1->mul;
...
void inoculation(Container& medium, Solid& sample1, float temp, Time* time1, int shaking_type)
{
 ...
 {
  case 1:
   qts_write(qts, QString("<font color=#357EC7>%1</font> and incubate with shaking for ").arg(sample1.new_name));
   time1->display_time();
   qts_write(qts, QString(" at <b><font color=#357EC7>%1%2C</font></b>.<br>".arg(temp).arg(0x00B0));
   break;
  case 2:
   qts_write(qts, QString("<font color=#357EC7>%1</font> and incubate without shaking for ").arg(sample1.new_name));
   time1->display_time();
   qts_write(qts, QString(" at <b><font color=#357EC7>%1%2C</font></b>.<br>".arg(temp).arg(0x00B0));
   break;
  default:qts_write(qts, "Invalid inputs.");
...
void inoculation(Container& medium, Solid& sample1, float temp, Time* time1, float cell_density, int shaking_type)
{
 ...
 {
  case 1:
   qts_write(qts, QString("<font color=#357EC7>%s</font> and incubate with shaking for ", sample1.new_name);
   time1->display_time();
   qts_write(qts, QString(" at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
   break;
  case 2:
   qts_write(qts, QString("<font color=#357EC7>%s</font> and incubate without shaking for ", sample1.new_name);
   time1->display_time();
   qts_write(qts, " at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
   break;
  default:qts_write(qts, "Invalid inputs.");
 ...
 qts_write(qts, "Calibrate the spectrophotometer and make sure that the cell density of the culture has reached <b><font color=#357EC7>%g X 10<sup>6</sup> cells/ml</font></b> before continuing.<br>", cell_density);
 if(spectrophotometer_no == 1)
 {
  equipments[equip_no] = "Spectrophotometer";
  equip_no++;
...
void store(Container& container1, float temp)
{
 // graph maintenance
 {
  ...
  if(temp == ON_ICE)
  {
   qts_write(qts, "Store ");
   check_container(container1);
   qts_write(qts, " <b><font color=#357EC7>on ice</font></b>.<br>");
  }
  else if(temp == BOILING_WATER)
   qts_write(qts, "Immerse the tube in boiling water.");
  else 
  {
   qts_write(qts, "Store ");
   check_container(container1);
...
void store_for(Container& container1, float temp, Time* time1)
{
 if(temp == ON_ICE)
 {
  qts_write(qts, "Store ");
  check_container(container1);
  qts_write(qts, " <b><font color=#357EC7>on ice</font></b> for ");
 }
 else if(temp == BOILING_WATER)
 {
  qts_write(qts, "Immerse ");
  check_container(container1);
  qts_write(qts, " in boiling water for ");
 }
 else if (temp == RT)
 {
  qts_write(qts, "Store ");
 ...
 else 
 {
  qts_write(qts, "Store ");
  check_container(container1);
  qts_write(qts, QString(" at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
 }
 time1->display_time();
 qts_write(qts, ".<br>");
 // graph maintenance
...
void store_for(Container& sample1, float temp, Time* time1, enum func function)
{
 if(function == 1)
 {
  qts_write(qts, "Denature ");
  check_container(sample1);
  qts_write(qts, QString(" at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
 }
 else if (function == 2)
 {
  qts_write(qts, "Perform enzyme inactivation by storing ");
  check_container(sample1);
  qts_write(qts, QString(" at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
 }
 time1->display_time();
...
void store_until(Container& container1, float temp, enum until type)
{
 // graph maintenance
 {
  ...
  switch(type)
  {
   case ETHANOL_EVAP:
    if (temp == RT)
      qts_write(qts, QString("Store %s at <b><font color=#357EC7>room temperature</font></b> until the ethanol has evaporated and no fluid is visible in the tube.<br>", container1.contents.new_name);
    else
      qts_write(qts, QString("Store %s at <b><font color=#357EC7>%g%cC</font></b> until the ethanol has evaporated and no fluid is visible in the tube.<br>", container1.contents.new_name, temp, 0x00B0);
    break;
   case OD:
    qts_write(qts, QString("Incubate %s at <b><font color=#357EC7>%g%cC</font></b> until the O.D.600 reaches 0.6.<br>", container1.contents.new_name, temp, 0x00B0);
    break;
   case THAW:
    qts_write(qts, QString("Allow %s to thaw at <b><font color=#357EC7>room temperature</font></b>.<br>", container1.contents.new_name);
    break;
   case COOLED:
    qts_write(qts, QString("Keep %s at <b><font color=#357EC7>room temperature</font></b> until cooled.<br>", container1.contents.new_name);
    break;
   case COLOUR_DEVELOPS:
    qts_write(qts, "Wait for the colour to develop.<br>");
    break;
   case THAW_ICE:
    qts_write(qts, QString("Allow %s to thaw on <b><font color=#357EC7>ice</font></b>.<br>", container1.contents.new_name);
    break;
   default: 
    break;
  }
...
void store_until(Container& container1, float temp, enum until type, Time* time1)
{
 switch(type)
 {
  case ETHANOL_EVAP:
   if (temp == RT)
     qts_write(qts, QString("Store %s at <b><font color=#357EC7>room temperature</font></b> until the ethanol has evaporated and no fluid is visible in the tube (~", container1.contents.new_name);
   else 
     qts_write(qts, QString("Store %s at <b><font color=#357EC7>%g%cC</font></b> until the ethanol has evaporated and no fluid is visible in the tube (~", container1.contents.new_name);
   break;
  case OD:
   qts_write(qts, QString("Incubate %s at <b><font color=#357EC7>%g%cC</font></b> until the O.D.600 reaches 0.6 (~", container1.contents.new_name);
   break;
  case THAW:
   qts_write(qts, QString("Allow %s to thaw at <b><font color=#357EC7>room temperature</font></b> (~", container1.contents.new_name);
   break;
  case COOLED:
   qts_write(qts, QString("Keep %s at <b><font color=#357EC7>room temperature</font></b> until cooled (~", container1.contents.new_name);
   break;
  case COLOUR_DEVELOPS:
   qts_write(qts, QString("Wait for the colour to develop (~", container1.contents.new_name);
   break;
  default:
   break;
 }
...
void invert_dry(Container& container1, float temp, Time* time1)
{
 if(temp == RT)
 {
  qts_write(qts, "Stand the tube containing %s for ", container1.contents.new_name);
  time1->display_time();
  qts_write(qts, " in an inverted position on a paper towel to allow all of the fluid to drain away.<br>");
 }
 else 
 {
  qts_write(qts, "Stand the tube containing %s in an inverted position on a paper towel to allow all of the fluid to drain away. Dry at <b><font color=#357EC7>%g%cC</font></b> for ", container1.contents.new_name, temp, 0x00B0);
  time1->display_time();
  qts_write(qts, ".<br>");
 }
 container1.volume = 0;
...
void invert_dry(Container& container1, float temp)
{
 // graph maintenance
 {
  ...
  if(temp == RT)
   qts_write(qts, "Stand the tube containing %s in an inverted position on a tissue paper to allow all of the fluid to drain away.<br>", container1.contents.new_name);
  else 
   qts_write(qts, "Stand the tube containing %s in an inverted position on a tissue paper to allow all of the fluid to drain away.<br>", container1.contents.new_name);
  container1.volume = 0;
}
...
void incubate(Container& container1, float temp, Time* time1, int rpm)
{
 ...
 else if((temp == ON_ICE)||(temp == 0))
 {
  qts_write(qts, "Incubate ");
  check_container(container1);
  qts_write(qts, " on <b><font color=#357EC7><b><font color=#357EC7>ice</font></b></font></b> for ");
 }
 else
 {
  qts_write(qts, "Incubate ");
  check_container(container1);
  qts_write(qts, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
 }
 time1->display_time();
 qts_write(qts, " with shaking at %d rpm.<br>", rpm);
 if(incubator_no == 1)
 {
  equipments[equip_no] = "Incubator";
  equip_no++;
...
void incubate_and_mix(Container& container1, float temp, Time* time1, Time* time_mix, enum mixing type)
{
 ...
 check_container(container1);
 switch (type){
   case STIRRING: qts_write(qts, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0); time1->display_time(); qts_write(qts, ", mixing gently by stirring the tube every "); time_mix->display_time(); qts_write(qts, ". <br>");break;
   case INVERTING: qts_write(qts, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0); time1->display_time(); qts_write(qts, ", mixing gently by inverting the tube every "); time_mix->display_time(); qts_write(qts, ". <br>");break;
   case VORTEXING: qts_write(qts, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0); time1->display_time(); qts_write(qts, ", mixing gently by vortexing the tube every "); time_mix->display_time(); qts_write(qts, ". <br>");break;
   default:break;
 }
 if(incubator_no == 1)
 {
...
void nanodrop(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Measure the concentration of %s in your final samples with the Nanodrop.<br>", container1.contents.new_name);
}
...
void time_constraint(Container& container1, Time* time1, enum time_constraint_type type)
{
 switch (type){
  case CURRENT_FLUID: if (time1 == 0)
       qts_write(qts, "<font color=red>NOTE: Use %s ", container1.contents.new_name);
       else
       {
        qts_write(qts, "<font color=red>NOTE: Use %s within ", container1.contents.new_name);
  };time1->display_time();qts_write(qts, ".</font><br>");break;
  case NEXTSTEP: if (time1 == 0)
       qts_write(qts, "<font color=red>NOTE: Proceed to the next step ");
      else
       qts_write(qts, "<font color=red>NOTE: Proceed to the next step within ");
   time1->display_time();qts_write(qts, "!</font><br>");break;
  default:break;
 }
 // graph maintenance
...
{
 qts_write(qts, "NOTE: If %s is/are used within ", container1.contents.new_name);
 time1->display_time();
 qts_write(qts, ", set temperature to <b><font color=#357EC7>%g%cC</font></b> and use, else store it at <b><font color=#357EC7>%g%cC</font></b>.", temp_use, 0x00B0, temp_store, 0x00B0);
 // graph maintenance
 {
...
void mixing_table(int no_rows, int no_columns, Volume* rxn_vol, Fluid array1[], char* tube[], float *vol, Container& container1)
{
 ...
 if(no_rows-1 == 1)
  qts_write(qts, "Use the following table as a checklist for preparing the reaction in %s:<br><br>", container1.name);
 else 
  qts_write(qts, "Use the following table as a checklist for preparing the reactions in %s:<br><br>", container1.name);
 qts_write(qts, "<table border cellpadding=5 rules=all frame=void bordercolor=#357EC7>");
 for(i=0; i<no_rows; i++)
 {
  for(j=0; j<no_columns; j++)
  {
   if((i == 0) && (j == 0))
    qts_write(qts, "<thead><tr><td>&nbsp;</td>");

   else if((i == 0) && (j != 0))
   {

    qts_write(qts, "<td><font color=#357EC7>%s</font></td>", (array1[j-1]).new_name);

    if (j == no_columns-1)
     qts_write(qts, "</tr></thead>");
   }
   else if((i != 0) && (j == 0))
    qts_write(qts, "<tbody><tr><td><font color=#357EC7>%s</font></td>", tube[i-1]);

   else 
   {
    if((i == no_rows-1 )&& (j == no_columns-1) && ((*(vol+(no_columns-1)*(i-1)+(j-1)) == XVAL)))
    {
     qts_write(qts, "<td><b><font color=#357EC7>Make up the volume to ");
     rxn_vol->display_vol();
     qts_write(qts, "</font></b></td>");
    }

    else if (*(vol+(no_columns-1)*(i-1)+(j-1)) == NA)
     qts_write(qts, "<td><b><font color=#357EC7>--</font></b></td>"); 
    else if (*(vol+(no_columns-1)*(i-1)+(j-1)) == XVAL)
     qts_write(qts, "<td><b><font color=#357EC7>X</font></b></td>");
    else
     qts_write(qts, "<td><b><font color=#357EC7>%g</font></b></td>", *(vol+(no_columns-1)*(i-1)+(j-1)));
    if (j == no_columns-1)
     qts_write(qts, "</tr></body>");
 ...
 for(i=1; i<no_rows; i++)
 {
  for(j=1; j<no_columns; j++)
  {
   ...
   {
    qts_write(qts, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", array1[j-1].new_name);
 ...
 if (container1.volume > ((no_rows-1)*rxn_vol->value))
  qts_write(qts, "<font color = red>Warning: The total reaction volume is greater than the required reaction volume.</font><br>");
 else if (container1.volume < ((no_rows-1)*rxn_vol->value))
  qts_write(qts, "<font color = red>Warning: The total reaction volume is less than the required reaction volume.</font><br>");
...
void mixing_table(int no_rows,int no_columns, Fluid array1[], char* tube[], Volume* volumes[], Volume* rxn_vol, Container& container1)
{
 ...
 for (i=0; i<no_columns-1;i++)
 {
  ...
  else
  {
   ...
   {
    qts_write(qts, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", array1[i].new_name);

   }
   else if (volumes[i]->value < 0)
   {
    qts_write(qts, "<font color = red>Warning: Negative volume input.</font><br>");
    break;
   }
   else
   {
...
void plate_out(Plate& plate1, Container& container1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  {
   qts_write(qts, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", container1.contents.new_name);
  ...
  qts_write(qts, "Plate out ");
  volume1->display_vol();
  qts_write(qts, " of %s onto ", container1.contents.new_name);
  if(plate1.state == "")
   qts_write(qts, "<font color=#357EC7>%s</font>.<br>", plate1.new_name);
  else
   qts_write(qts, "<a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", plate1.new_name, plate1.new_name);
  container1.volume = container1.volume - volume1->value * volume1->mul;
}
...
void plate_out(Plate& plate1, Container& container1)
{
 // graph maintenance
 {
  ...
  if(plate1.state == "") 
   qts_write(qts, "Plate out %s onto <font color=#357EC7>%s</font>.<br>", container1.contents.new_name, plate1.new_name);
  else
   qts_write(qts, "Plate out %s onto <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", container1.contents.new_name, plate1.new_name, plate1.new_name);
  container1.volume = 0;
}

void name_container(Container& container1, char* name)
{
 qts_write(qts, "Set aside a fresh %s. Call it %s. <br>", container1.name, name);
...
void thermocycler (Container& master_mix, int cycles, float temp1, Time* time1, float temp2, Time* time2, float temp3, Time* time3, enum pcr_type type)
{
 ...
 if(type == NORMAL)
 {
  qts_write(qts, "Thermocycling<br><ul><li>No. of cycles: <b><font color=#357EC7>%d</font></b></li>", cycles);
  qts_write(qts, "<li>Denature: <b><font color=#357EC7>%g%cC</font></b>, ", temp1, 0x00B0);
  time1->display_time();
  qts_write(qts, "</li> <li> Anneal: <b><font color=#357EC7>%g%cC</font></b>, ", temp2, 0x00B0);
  time2->display_time();
  qts_write(qts, "</li> <li>Elongate: <b><font color=#357EC7>%g%cC</font></b>, ", temp3, 0x00B0);
  time3->display_time();
  qts_write(qts, "</li></ul>");
  ...
  qts_write(qts, "Carry out colony PCR of %s.<br>", master_mix.contents.new_name);
...
void thermocycler (Container& master_mix, int cycles, float temp1, Time* time1, float temp2, Time* time2, float temp3, Time* time3, enum pcr_type type, float grad_temp)
{
 ...
 else if(type == GRADIENT)
 {
  ...
  qts_write(qts, "<li>Denature: <b><font color=#357EC7>%g%cC</font></b>, ", temp1, 0x00B0);
  time1->display_time();
  qts_write(qts, "</li> <li> Anneal: <b><font color=#357EC7>%g%cC- %g%cC (gradient PCR)</font></b>, ", temp2, 0x00B0, grad_temp, 0x00B0);
  time2->display_time();
  qts_write(qts, "</li> <li>Elongate: <b><font color=#357EC7>%g%cC</font></b>, ", temp3, 0x00B0);
  time3->display_time();
  qts_write(qts, "</li></ul>");
...
{
 qts_write(qts, "Set the thermocycler to run the following program:<br>");

 qts_write(qts, "<ul><li><b><font color=#357EC7>%g%cC</font></b>, ", temp1, 0x00B0);
 time1->display_time();
 qts_write(qts, "</li> <li><b><font color=#357EC7>%g%cC</font></b>, ", temp2, 0x00B0);
 time2->display_time();
 qts_write(qts, "</li></ul>");
 if(thermocycler_no == 1)
 {
...
void thermocycler (Plate& plate1, enum pcr_type type)
{
 // graph maintenance
 {
  ...
  if(type == COLONY)
   qts_write(qts, "Carry out colony PCR on %s.<br>", plate1.new_name);
...
void ce_detect (Container& container1, float length, float volt_per_cm, Fluid& fluid1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Detect/separate %s by capillary electrophoresis with the following settings - <b><font color=#357EC7>%g</font></b> cm at <b><font color=#357EC7>%g</font></b> V/cm using %s.<br>", container1.contents.new_name, length, volt_per_cm, fluid1);
...
void ce_detect (Container& container1, float length, float volt_per_cm, Fluid& fluid1, Time* time1)
{
 ...
 qts_write(qts, "Detect/separate %s by capillary electrophoresis with the following settings - <b><font color=#357EC7>%g</font></b> cm at <b><font color=#357EC7>%g</font></b> V/cm using %s for ", container1.contents.new_name, length, volt_per_cm, fluid1.new_name);
 time1->display_time();
 qts_write(qts, ".<br>");
 name_sample(container1, "separated flow");
 // graph maintenance
...
void measure_fluorescence (Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Measure the fluorescence of %s.<br>", container1.contents.new_name);
...
void centrifuge_column(Column& column1, Speed* speed1, float temp, Time* time1)
{
 ...
 if(temp==RT);
  ...
  qts_write(qts, "at <b><font color=#357EC7>%g%cC</font></b>s", temp, 0x00B0);
 qts_write(qts, ".<br>");
 if(centrifuge_no == 1)
 {
  equipments[equip_no] = "Centrifuge";
 ...
 qts_write(qts, "Discard the flow-through.<br>"); 
 column1.volume = 0;
...
void centrifuge_flow_through(Column& column1, Speed* speed1, float temp, Time* time1, Container& container1)
{
 ...
 qts_write(qts, "Discard the column.<br>");
 if(centrifuge_no == 1)
 {
  equipments[equip_no] = "Centrifuge";
  equip_no++;
...
void add_to_column(Column& column1, Fluid& fluid1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  if ((fluid1.volume == 0)|| (fluid1.volume < (volume1->value*volume1->mul)))
  {
   qts_write(qts, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", fluid1.new_name);
  ...
  volume1->display_vol();
  if(fluid1.state == "")
   qts_write(qts, " of <font color=#357EC7>%s</font> to %s.<br>", fluid1.new_name, column1.name);
  else 
   qts_write(qts, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> to %s.<br>", fluid1.new_name, fluid1.new_name, column1.name);
...
void add_to_column(Column& column1, Container& container1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  if(container1.contents.state == "")
   qts_write(qts, " of <font color=#357EC7>%s</font> %s.<br>", container1.contents.new_name);
  else
   qts_write(qts, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> %s.<br>", container1.contents.new_name, container1.contents.new_name);
...
void add_to_column(Column& column1, Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Add <font color=#357EC7>%s</font> to %s.<br>", container1.contents.new_name, column1.name);
  column1.contents = container1.contents;
...
void transfer_column(Column& column1, Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Transfer %s into %s.<br>", column1.name, container1.name);
}
...
void vortex_column(Column& column1)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Vortex %s gently.<br>",column1.name);
}
...
void drain(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Drain %s.<br>", container1.name);
  container1.volume = 0;
}
...
void wash_cells(Container& chamber, Fluid& sample1)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Wash cells with %s.<br>", sample1.new_name);
}
...
void collect_tissue(Tissue& sample1, float wt, enum weight_unit unit, Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Collect <b><font color=#357EC7>%g %s</font></b> of %s in  %s. <br>", wt, wu, sample1.new_name, container1.name);
...
void collect_tissue(Tissue& sample1, Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Collect %s into %s and snap-freeze in liquid nitrogen.<br>", sample1.new_name, container1.name);
...
void homogenize_tissue(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Pre-cool the mortars by pouring liquid nitrogen into them.<br>");
  qts_write(qts, "Add %s into the mortar containing liquid nitrogen and grind with a pestle until it is a fine powder.<br>", container1.contents.new_name);
  comment("Use gloves!");
  if(mortar_no==1)
  {
...
void homogenize_tissue(Container& container1, Fluid& fluid1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Homogenize %s in %s.<br>", container1.contents.new_name, fluid1.new_name);
  name_sample(container1, "homogenized tissue");
...
void dry_pellet(Container& container1, drying type, Time* time1)
{
 ...
 qts_write(qts,"Dry the pellet %s for ", additional);
 time1->display_time();
 qts_write(qts, ".<br>");
 container1.volume = 0;
 // graph maintenance
...
void dry_pellet(Container& container1, drying type)
{
 ...
 // graph maintenance
 {
  ...
  qts_write(qts,"Dry the pellet %s. <br>",additional);
  container1.volume = 0;
}
...
void mixing_table_pcr(int no_rows, Volume* rxn_vol, Fluid array1[], char*initial_conc[], char* final_conc[], Volume* vol[], Container& container1)
{
 ...
 qts_write(qts,"Set up a reaction in %s as follows on ice:<br><br>", container1.name);
 qts_write(qts,"<table border cellpadding=5 rules=all frame=void bordercolor=#357EC7>");
 for(i=0;i<no_rows;i++)
 {
  for(j=0;j<4;j++)
  {
   ...
   else if((i==0)&&(j!=0))
   {
    if(j==1)
    {
     qts_write(qts,"<td><font color=#357EC7>Initial concentration</font></td>");
    }
    if(j==2)
     qts_write(qts,"<td><font color=#357EC7>Final concentration</font></td>");
    if (j==3)
    {
     qts_write(qts,"<td><font color=#357EC7>Final volume per ");
     rxn_vol->display_vol();
     qts_write(qts," reaction</font></td>");
     qts_write(qts,"</tr></thead>");
    }
   }
   else if((i!=0)&&(j==0))
    qts_write(qts,"<tbody><tr><td><font color=#357EC7>%s</font></td>",array1[i-1].new_name);
   else 
   {
    if(j==1)
    {
     qts_write(qts,"<td><b><font color=#357EC7>%s</font></b></td>",initial_conc[i-1]);
    }
    if(j==2)
    {
     qts_write(qts,"<td><b><font color=#357EC7>%s</font></b></td>",final_conc[i-1]);
    }
    if(j==3)
    {
     ...
     {
      make_vol_up = 1;
      qts_write(qts,"<td><b><font color=#357EC7>Make up volume to ");
      rxn_vol->display_vol();
      qts_write(qts, "</font></b></td>");
     }
...
void electrophoresis(int no_samples, Container container_array[], float agar_conc, Volume* pdt_vol, Volume* loading_buff_vol)
{
 ...
 // graph maintenance
 {
  ...
  qts_write(qts,"Perform %g&#37 agarose gel electrophoresis of ",agar_conc);
  if(no_samples == 2)
   qts_write(qts, "%s and %s (", container_array[0].contents.new_name, container_array[1].contents.new_name);
  else
  {
   for(i=0; i<no_samples;i++)
   {
    if(i=no_samples-1)
     qts_write(qts, "%s (", container_array[i].contents.new_name);        
    else if(i=no_samples-2)
     qts_write(qts, "%s and ", container_array[i].contents.new_name);
    else
     qts_write(qts, "%s, ", container_array[i].contents.new_name);
  ...
  pdt_vol->display_vol();
  qts_write(qts, " sample and ");
  loading_buff_vol->display_vol();
  qts_write(qts, " loading buffer), mixed with ethidium bromide and visualize with UV transilluminator to confirm the presence of required product.<br>");
...
void electrophoresis(Container& container1,float agar_conc)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Perform %g&#37 agarose gel electrophoresis of appropriate quantity of  %s mixed with ethidium bromide and visualize with UV transilluminator to confirm the presence of required product.<br>",agar_conc, container1.contents.new_name);
  if(electrophoresis_no==1)
  {
   equipments[equip_no]="Electrophoretic unit";
   equip_no++;
...
void electrophoresis(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Perform agarose gel electrophoresis of appropriate quantity of  %s mixed with ethidium bromide and visualize with UV transilluminator to confirm the presence of required product.<br>", container1.contents.new_name);
  if(electrophoresis_no==1)
  {
   equipments[equip_no]="Electrophoretic unit";
   equip_no++;
...
{
 qts_write(qts, "Program a standard thermocycler to run the reaction using the following parameters:<br>");
 qts_write(qts, "Initial denaturation<br><ul><li>Denature: <b><font color=#357EC7>%g%cC</font></b>, ", temp, 0x00B0);
 time1->display_time();
 qts_write(qts, "</li></ul>");
 // graph maintenance
 {
...
{
 qts_write(qts,"Termination<ul>");
 qts_write(qts,"<li>Elongate: <b><font color=#357EC7>%g%cC</font></b>, ", temp_ext, 0x00B0); 
 time_ext->display_time();
 qts_write(qts, "</li><li>Halt reaction: <b><font color=#357EC7>%g%cC</font></b>, ", temp_halt, 0x00B0);
 time_halt->display_time();
 qts_write(qts, "</li><li>Hold: <b><font color=#357EC7>%g%cC</font></b>, until removed from machine </li></ul>", temp_hold, 0x00B0);
 // graph maintenance
 {
...
void centrifuge_phases_bottom(Container& container1, Speed* speed1, float temp, Time* time1, Volume* aspirate_vol, Volume* leave_behind)
{
 ...
 qts_write(qts, " for ");
 time1->display_time();
 qts_write(qts, " and gently aspirate out ");
 aspirate_vol->display_vol();
 qts_write(qts, " of the supernatant and discard it.<br>Leave behind ");
...
void patch(Plate& plate1,Plate& colony)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Patch <font color=#357EC7>%s</font> onto %s.<br>", colony.new_name, plate1.new_name);
}
...
void incubate_plate(Plate& plate1, float temp, Time* time1)
{
 ...
 if(temp == RT)
  qts_write(qts, "Incubate %s at <b><font color=#357EC7><b><font color=#357EC7>room temperature</font></b></font></b> for ", plate1.new_name);
 else if((temp == ON_ICE)||(temp == 0))
  qts_write(qts, "Incubate %s on <b><font color=#357EC7><b><font color=#357EC7>ice</font></b></font></b> for ", plate1.new_name);
 else
  qts_write(qts, "Incubate %s at <b><font color=#357EC7>%g%cC</font></b> for ", plate1.new_name, temp, 0x00B0);
 time1->display_time();
 qts_write(qts, ".<br>");
 if(incubator_no == 1)
 {
...
void sequencing(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Dilute %s to <font color=#357EC7>100ng/ \B5l</font> and send <font color=#357EC7>1 \B5g (10 \B5L)</font> for sequencing.<br>", container1.contents.new_name);
}
...
void weigh(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Weigh the amount of %s present.<br>", container1.contents.new_name);
}

void thermocycler_anneal(Container& container1, float cool_to_temp, float gradient, Time* time1)
{
 if (cool_to_temp==RT)
  qts_write(qts,"Cool %s to room temperature at the rate of <b><font color=#357EC7>%g%cC</font></b> every ", container1.contents.new_name, gradient, 0x00B0);
 else
  qts_write(qts,"Cool %s to <b><font color=#357EC7>%g%cC</font></b> at the rate of <b><font color=#357EC7>%g%cC</font></b> every ", container1.contents.new_name, cool_to_temp, 0x00B0, gradient, 0x00B0);
 time1->display_time();
 qts_write(qts, ".<br>");
 // graph maintenance
 {
...
void immerse_slide(Slide& slide1,Fluid& sample1, Time* time1, enum func function, int no_of_times)
{
 ...
 if (sample1.state == "")
 {
  switch(function)
  {
   case DEWAX: qts_write(qts, "Dewax %s in <b><font color=#357EC7>%s</font></b> for ",slide1.name, sample1.new_name);break;
   default:break;
  }
 }
 else{
  switch(function)
  {
   case DEWAX: qts_write(qts, "Dewax %s in <a href=\"#%s\" >%s</a> for ", slide1.name, sample1.new_name, sample1.new_name);break;
   default:break;
  }
 }
 time1->display_time();
 qts_write(qts, " %s.<br>", noft);
 ttr2 = total_time_required-ttr1;
...
void immerse_slide(Slide& slide1, Fluid& sample1, Time* time1, int no_of_times)
{
 ...
 if (sample1.state == "")
  qts_write(qts, "Immerse %s in <b><font color=#357EC7>%s</font></b> for ", slide1.name, sample1.new_name);
 else 
  qts_write(qts, "Immerse %s in <a href=\"#%s\" >%s</a> for ", slide1.name, sample1.new_name, sample1.new_name);
 time1->display_time();
 qts_write(qts, " %s.<br>", noft);
 ttr2 = total_time_required-ttr1;
 total_time_required = total_time_required + ttr2 * no_of_times;
...
void immerse_slide(Slide& slide1,Fluid& sample1, int no_of_times)
{
 // graph maintenance
 {
  ...
  if ((sample1.state == "")&&(no_of_times == 1))
   qts_write(qts, "Immerse %s in %s.<br>", slide1.name, sample1.new_name);
  else if ((sample1.state == "")&&(no_of_times > 1))
   qts_write(qts, "Immerse %s in %s %s.<br>", slide1.name, sample1.new_name, noft);
  else if (no_of_times > 1)
   qts_write(qts, "Immerse %s in <a href=\"#%s\" >%s</a> %s.<br>", slide1.name, sample1.new_name, sample1.new_name, noft);
  else 
   qts_write(qts, "Immerse %s in <a href=\"#%s\" >%s</a>.<br>", slide1.name, sample1.new_name, sample1.new_name);
  ttr2 = total_time_required-ttr1;
  total_time_required = total_time_required + ttr2 * no_of_times;
}
...
void remove_slide(Slide& slide1, Fluid& sample1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Take out %s from %s.<br>", slide1.name, sample1.new_name);
}
...
void wash_tissue(Tissue& tissue1, Fluid& sample1, enum wash_type type, float temp, int shaking)
{
 // graph maintenance
 {
  ...
  if(sample1.state == "")
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <font color=#357EC7>%s</font>", tissue1.new_name, sample1.new_name);break;
    case RINSING:qts_write(qts, "Rinse %s in <font color=#357EC7>%s</font>", tissue1.new_name, sample1.new_name);break;
    default:qts_write(qts, "Wash %s in <font color=#357EC7>%s</font>", tissue1.new_name, sample1.new_name);break;
   }
  }
  else
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <a href=\"#%s\" >%s</a>", tissue1.new_name, sample1.new_name, sample1.new_name);break;
    case RINSING:qts_write(qts, "Rinse %s in <a href=\"#%s\" >%s</a>", tissue1.new_name, sample1.new_name, sample1.new_name);break;
    default:qts_write(qts, "   Wash %s in <a href=\"#%s\" >%s</a>", tissue1.new_name, sample1.new_name, sample1.new_name);break;
   }
  }
  if (temp == RT)
   qts_write(qts, " at <b><font color=#357EC7>room temperature</font></b>%s.<br>", shake_type);
  else
   qts_write(qts, " at <b><font color=#357EC7>%g%cC</font></b>%s.<br>", temp, 0x00B0, shake_type);
  if(shaking == 1)
  {
   if(shaker_no == 1)
   {
...
void wash_tissue(Tissue& tissue1, Fluid& sample1, enum wash_type type, Time* time1, int no_of_times, float temp, int shaking)
{
 ...
 if(sample1.state == "")
 {
  if (no_of_times == 1)
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <font color=#357EC7>%s</font> for ", tissue1.new_name, sample1.new_name);break;
    case RINSING:qts_write(qts, "Rinse %s in <font color=#357EC7>%s</font> for ", tissue1.new_name, sample1.new_name);break;
    default:break;
   }
   time1->display_time();
   if (temp == RT)
  ...
  else
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <font color=#357EC7>%s</font>, <b><font color=#357EC7>%s</font></b> for ", tissue1.new_name, sample1.new_name, noft);break;
    case RINSING:qts_write(qts, "Rinse %s in <font color=#357EC7>%s</font>, <b><font color=#357EC7>%s</font></b> for ", tissue1.new_name, sample1.new_name, noft);break;
    default:break;
   }
   time1->display_time();
   if (temp == RT)
    qts_write(qts, " each at <b><font color=#357EC7>room temperature</font></b>%s.<br>", shake_type);
   else
    qts_write(qts, " each at <b><font color=#357EC7>%g%cC</font></b>%s.<br>", temp, 0x00B0, shake_type);
  }
 }
 else 
 {
  if (no_of_times == 1)
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <a href=\"#%s\" >%s</a> for ", tissue1.new_name, sample1.new_name, sample1.new_name);break;
    case RINSING:qts_write(qts, "Rinse %s in <a href=\"#%s\" >%s</a> for ", tissue1.new_name, sample1.new_name, sample1.new_name);break;
    default:break;
   }
   time1->display_time();
   if (temp == RT)
  ...
  else
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <a href=\"#%s\" >%s</a>, <b><font color=#357EC7>%s</font></b> for ",tissue1.new_name, sample1.new_name, sample1.new_name, noft);break;
    case RINSING:qts_write(qts, "Rinse %s in <a href=\"#%s\" >%s</a>, <b><font color=#357EC7>%s</font></b> for ",tissue1.new_name, sample1.new_name, sample1.new_name, noft);break;
    default:break;
   }
   time1->display_time();
   if (temp == RT)
...
void immerse_slide_and_incubate(Slide& slide1, Fluid& sample1, float temp, Time* time1)
{
 ...
 qts_write(qts, "Incubate %s for ", slide1.name);
 time1->display_time();
 if(sample1.state == "")
 {
  if(temp==RT)
   qts_write(qts, " in <b><font color=#357EC7>%s</font></b> at <b><font color=#357EC7>room temperature</font></b>.<br>", sample1.new_name);
  else
   qts_write(qts, " in <b><font color=#357EC7>%s</font></b> at <b><font color=#357EC7>%g%cC</font></b>.<br>", sample1.new_name, temp, 0x00B0);
 }
 else
 {
  if(temp==RT)
   qts_write(qts, " in <a href=\"#%s\" >%s</a> at <b><font color=#357EC7>room temperature</font></b>.<br>", sample1.new_name, sample1.new_name);
  else
   qts_write(qts, " in <a href=\"#%s\" >%s</a> at <b><font color=#357EC7>%g%cC</font></b>.<br>", sample1.new_name, sample1.new_name, temp, 0x00B0);
 }
 // graph maintenance
 {
  ...
  case 5:
   ...
   if(sample1.new_name == "")
   {
    if(type == HUMIDIFIED_CHAMBER)
     qts_write(qts, " in <b><font color=#357EC7>%s</font></b> at <b><font color=#357EC7>room temperature</font></b> in humidified chamber.<br>", sample1.new_name);
   }
   else
   {
    if(type == HUMIDIFIED_CHAMBER)
     qts_write(qts, " in <a href=\"#%s\" >%s</a> at <b><font color=#357EC7>room temperature</font></b> in humidified chamber.<br>", sample1.new_name, sample1.new_name);
   }
   break;
  default:break;
 }
...
void add_to_slide(Slide& slide1, Fluid& sample1, Volume* volume1)
{
 // graph maintenance
 {
  ...
  if ((sample1.volume == 0)|| (sample1.volume < (volume1->value*volume1->mul)))
  {
   qts_write(qts, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", sample1.new_name);
  ...
  if(sample1.state == "")
  {
   qts_write(qts, "Add ");
   volume1->display_vol();
   qts_write(qts, " of <b><font color=#357EC7>%s</font></b> to each of the %s.<br>", sample1.new_name, slide1.name);
  }
  else
  {
   qts_write(qts, "Add ");
   volume1->display_vol();
   qts_write(qts, " of <a href=\"#%s\" >%s</a> to each of the %s.<br>", sample1.new_name, sample1.new_name, slide1.name);
  }
}
...
void dry_slide(Slide& slide1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Dry %s.<br>", slide1.name);
}
...
void mount_observe_slide(Slide& slide1, Fluid& fluid1, char* device)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Mount %s with <b><font color=#357EC7>%s</font></b>, let it solidify and view under %s.<br>", slide1.name, fluid1.new_name, device);
}
...
void facs(Container& container1)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "FACS: sort %s based on fluorescence.", container1.contents.new_name);
}
...
void cell_culture(Container& cells,Fluid& medium, int centri_speed, float temp, float time, float percent_CO2, Fluid& for_wash_valves, Fluid& for_wash_chambers, Fluid& for_trypsinization, float for_feeding)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Perform cell culture with the specified parameters.");
}
...
void transfection(Container& container1, Fluid& medium, Fluid& dna)
{
 // graph maintenance
 {
  ...
  qts_write(qts,"Transfect %s with %s.", container1.contents.new_name, dna.new_name);
}
...
void electroporate (Container& container1, float voltage, int no_pulses)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Set the electroporator to deliver <b><font color=#357EC7>%g V</font></b>, and then press the PULSE button <b><font color=#357EC7>%d</font></b> times. <br>", voltage, no_pulses);
  if(electro_no==1)
  {
...
void store_plate(Plate& plate1, float temp)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Store %s at <b><font color=#357EC7>%g%cC</font></b>.<br>", plate1.new_name, temp, 0x00B0);
}
...
void wash_slide(Slide& slide1, Fluid& sample1, enum wash_type type)
{
 ...
 if(sample1.state == "")
 {
  switch(type)
  {
   case WASHING:qts_write(qts, "Wash %s in <b><font color=#357EC7>%s</font></b>.<br>", slide1.name, sample1.new_name);break;
   case RINSING:qts_write(qts, "Rinse %s in <b><font color=#357EC7>%s</font></b>.<br>", slide1.name, sample1.new_name);break;
   default:qts_write(qts, "Wash %s in <b><font color=#357EC7>%s</font></b>.<br>", slide1.name, sample1.new_name);break;
  }
 }
 else
 {
  switch(type)
  {
   case WASHING:qts_write(qts, "Wash %s in <a href=\"#%s\" >%s</a>.<br>", slide1.name, sample1.new_name, sample1.new_name);break;
   case RINSING:qts_write(qts, "Rinse %s in <a href=\"#%s\" >%s</a>.<br>", slide1.name, sample1.new_name,  sample1.new_name);break;
   default:qts_write(qts, "   Wash %s in <a href=\"#%s\" >%s</a>.<br>", slide1.name, sample1.new_name, sample1.new_name);break;
  }
 }
}
...
void wash_slide(Slide& slide1, Fluid& sample1, enum wash_type type, Time* time1, int no_of_times)
{
 ...
 if(sample1.state == "")
 {
  if (no_of_times == 1)
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <font color=#357EC7>%s</font> for ", slide1.name, sample1.new_name);break;
    case RINSING:qts_write(qts, "Rinse %s in <font color=#357EC7>%s</font> for ", slide1.name, sample1.new_name);break;
    default:break;
   }
   time1->display_time();
   qts_write(qts, ".<br>");
  ...
  else
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <font color=#357EC7>%s</font>, <b><font color=#357EC7>%s</font></b> for ", slide1.name, sample1.new_name, noft);break;
    case RINSING:qts_write(qts, "Rinse %s in <font color=#357EC7>%s</font>, <b><font color=#357EC7>%s</font></b> for ", slide1.name, sample1.new_name, noft);break;
    default:break;
   }
   time1->display_time();
   qts_write(qts, " each.<br>");
  }
 }
 else 
 {
  if (no_of_times == 1)
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <a href=\"#%s\" >%s</a> for ", slide1.name, sample1.new_name, sample1.new_name);break;
    case RINSING:qts_write(qts, "Rinse %s in <a href=\"#%s\" >%s</a> for ", slide1.name, sample1.new_name, sample1.new_name);break;
    default:break;
   }
   time1->display_time();
   qts_write(qts, ".<br>");
  ...
  else
  {
   switch(type)
   {
    case WASHING:qts_write(qts, "Wash %s in <a href=\"#%s\" >%s</a>, <font color=#357EC7>%s</font> for ", slide1.name, sample1.new_name, sample1.new_name, noft);break;
    case RINSING:qts_write(qts, "Rinse %s in <a href=\"#%s\" >%s</a>, <font color=#357EC7>%s</font> for ", slide1.name, sample1.new_name, sample1.new_name, noft);break;
    default:break;
   }
   time1->display_time();
   qts_write(qts, " each.<br>");
...
void measure_solid(Solid& solid1, float weight, enum weight_unit unit, Container& container1)
{
 ...
 default : type = "mg";mul = MICRO;break;
  ...
  {
   if (solid1.state == "")
    qts_write(qts, "Measure out <b><font color=#357EC7>%g %s</font></b> of <font color=#357EC7>%s</font> into %s.<br>", weight, type, solid1.new_name, container1.name);
   else
    qts_write(qts, "Measure out <b><font color=#357EC7>%g %s</font></b> of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", weight, type, solid1.new_name, solid1.new_name, container1.name);
   prev_container = container1.name;
   prev_cont++;
  }
  else if( prev_container == container1.name)
  {
   if (solid1.state == "")
    qts_write(qts, "Add <b><font color=#357EC7> %g %s</font></b> of <font color=#357EC7>%s</font>.<br>", weight, type, solid1.new_name);
   else
    qts_write(qts, "Add <b><font color=#357EC7> %g %s</font></b> of <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", weight, type, solid1.new_name, solid1.new_name);
  }
...
void measure_solid(Solid& solid1, Container& container1)
{
 // graph maintenance
 {
  ...
  {
   if (solid1.state == "")
    qts_write(qts, "Measure out <font color=#357EC7>%s</font> into %s.<br>", solid1.new_name, container1.name);
   else
    qts_write(qts, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", solid1.new_name, solid1.new_name, container1.name);
   prev_container = container1.name;
   prev_cont++;
  }
  else if( prev_container == container1.name)
  {
   if (solid1.state == "")
    qts_write(qts, "Add <font color=#357EC7>%s</font>.<br>", solid1.new_name);
   else
    qts_write(qts, "Add <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", solid1.new_name, solid1.new_name);
  }
  else if (solid1.state == "")
  {
   qts_write(qts, "Measure out <font color=#357EC7>%s</font> into %s.<br>", solid1.new_name, container1.name);
...
Symbol new_symbol(char* symbol, char* def)
{
 ...
 {
  qts_write(qts, "</ul><h2>Parameters:</h2><ul type=\"circle\">");
  print_parameters++;
 }
 qts_write(qts, "<li>%s - %s</li>", symbol, def);
 return s1;
}
...
void streak_plate(Plate& plate1, Plate& colony)
{
 // graph maintenance
 {
  ...
  qts_write(qts, "Use a sterile toothpick or disposable loop to transfer a small segement of %s to a streak or patch on %s. <br>", colony.new_name, plate1.new_name);
}
