 ...
 * Date:    2010/11/01                          \n
 *
 * $Id: mdsLoadJPEGDir.cpp 2098 2012-02-16 23:50:36Z spanel $
 *
 * File description:
...
int CLoadJPEGDir::loadImagesFromDir(ImageType& AuxImage)
{
    ...
    for( ; bResult; bResult = spFileBrowser->findNext(File) )
    {
        ...
        // Read JPEG image
        bool bLoaded = mds::img::loadJPEG(AuxImage, FileChannel);
        ...
        if( !bLoaded )
        {
            MDS_CERR('<' << this->m_sFilename << "> Failed to read input JPEG image: " << File.m_sName << std::endl);
            continue;
        }
...
void CLoadJPEGDir::writeExtendedUsage(std::ostream& MDS_UNUSED(Stream))
{
