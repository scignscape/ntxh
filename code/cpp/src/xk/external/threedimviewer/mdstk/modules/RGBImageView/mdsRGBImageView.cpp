 ...
 * Date:    2007/06/13                          \n
 *
 * $Id: mdsRGBImageView.cpp 2098 2012-02-16 23:50:36Z spanel $
 *
 * File description:
...
void CRGBImageView::writeExtendedUsage(std::ostream& MDS_UNUSED(Stream))
{
...
void createTexture(mds::img::CRGBImage *pRGBImage)
{
    ...
    for( mds::tSize j = 0; j < YSize; ++j )
    {
        ...
        {
            mds::img::tRGBPixel Value = pRGBImage->at(i,j);
            *(p++) = Value.r();
            *(p++) = Value.g();
            *(p++) = Value.b();
        }
