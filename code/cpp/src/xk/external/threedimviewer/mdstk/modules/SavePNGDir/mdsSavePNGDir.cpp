 ...
 * Date:    2010/11/01                          \n
 *
 * $Id: mdsSavePNGDir.cpp 2098 2012-02-16 23:50:36Z spanel $
 *
 * File description:
 * - Saves all input slices as PNG images to a given directory.
 */

#include "mdsSavePNGDir.h"
...
//! Module description
const std::string MODULE_DESCRIPTION    = "Module saves all input slices as PNG images to a directory";
...
const std::string DEFAULT_FILEEXT       = ".png";


//==============================================================================
/*
 * Implementation of the class CSavePNGDir.
 */
CSavePNGDir::CSavePNGDir(const std::string& sDescription)
...
CSavePNGDir::~CSavePNGDir()
{
}


bool CSavePNGDir::startup()
{
    // Note
...
bool CSavePNGDir::saveImageToDir(ImageType& Image)
{
    // Image label
    static int iCount = 0;
    ...
    // Save PNG image
    bool bSaved = mds::img::savePNG(Image, FileChannel);
    ...
    if( !bSaved )
    {
        MDS_CERR('<' << this->getFilename() << "> Failed to write the output PNG image: " << Stream.str() << std::endl);
        return false;
    }
...
void CSavePNGDir::writeExtendedUsage(std::ostream& MDS_UNUSED(Stream))
{
...
int main(int argc, char *argv[])
{
    ...
    CSavePNGDirPtr spModule(new CSavePNGDir(MODULE_DESCRIPTION));
