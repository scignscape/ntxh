...
template <typename T>
inline void CSparseMatrixAlt<T>::create(tSize rows, tSize cols)
{
    clear();
...
template <typename T>
inline void CSparseMatrixAlt<T>::create(tSize rows, tSize cols, const tElement& dominant)
{
    clear();
...
inline CSparseMatrixAlt<T>& CSparseMatrixAlt<T>::create(const CSparseMatrixBase<Derived>& Matrix)
{
    ...
    for( typename Derived::tIterator It(MatrixImpl); It; ++It )
    {
...
template <typename T>
inline CSparseMatrixAlt<T>& CSparseMatrixAlt<T>::operator =(const CSparseMatrixAlt& Matrix)
{
    m_Matrix = Matrix.m_Matrix;
    m_Dominant = Matrix.m_Dominant;
    m_NumOfRows = Matrix.m_NumOfRows;
...
template <typename T>
template <class Function>
inline Function CSparseMatrixAlt<T>::pforEachRef(Function Func)
{
...
template <typename T>
template <class Function>
inline Function CSparseMatrixAlt<T>::pforEach(Function Func) const
{
#pragma omp parallel for schedule(static) default(shared)
    for( tSize i = 0; i < m_NumOfCols; ++i )
    {
