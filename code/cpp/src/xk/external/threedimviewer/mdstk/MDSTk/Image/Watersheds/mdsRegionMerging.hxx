...
bool CRegionMerging<I,M>::operator()(const tImage& SrcImage, tRegionImage& RegionImage)
{
    ...
    MDS_LOG_NOTE("Initial number of regions = " << NumOfRegions);

    // Create the RAG (Region Adjacency Graph)
    typedef mds::img::CAdjacencyGraph<double> tRAG;
    tRAG RAG(NumOfRegions);

    // Initial mapping of regions
    mds::math::CVector<tRegionNumber> Mapping(NumOfRegions);
    ...
    // Image size
    tSize XSize = mds::math::getMin(SrcImage.getXSize(), RegionImage.getXSize());
    tSize YSize = mds::math::getMin(SrcImage.getYSize(), RegionImage.getYSize());
    ...
    // Image features
    typedef mds::img::CArray<tFeatureVector> tFeatures;
//    typedef std::vector<tFeatureVector> tFeatures;
...
//                RAG.set(r2, r1, calculate(Features[r2], Features[r1]));
    ...
    mds::math::CVector<tRegionNumber> SavedMapping(Mapping);

    // While any two regions can be merged
    double dMinSimilarity = 1.0e6;
//    for( ; Count > NumOfRegions; )
    for( ; Count > 0; )
    {
        ...
        MDS_LOG_NOTE("Maximal similarity = " << dMaxSimilarity << ", r1 = " << Max1 << ", r2 = " << Max2);
        ...
        if( /*NumOfRegions == -1 &&*/ Count <= MinCount )
        {
            ...
            MDS_LOG_NOTE("Min/Max similarity ratio = " << dDelta);
...
//            RAG.set(Max1, i, calculate(Features[Max1], Features[i]));
    ...
    MDS_LOG_NOTE("Final number of regions = " << NumOfRegions);
...
typename CRegionMerging<I,M>::tRegionNumber CRegionMerging<I,M>::reassignRegions(tRegionImage& RegionImage)
{
    ...
    tRegionNumber MaxRegion = mds::img::getMax<tRegionNumber>(RegionImage);
    if( MaxRegion <= 0 )
    {
        return 0;
    }
    ...
    // Mapping of regions
    mds::math::CVector<tRegionNumber> Mapping(MaxRegion + 2);
    Mapping.fill(0);
