 ...
 * Date:    2006/08/29                          \n
 *
 * $Id: mdsEM.hxx 2090 2012-02-14 00:21:31Z spanel $
 *
 * Description:
...
//    double dMDL = -DBL_MAX;
    ...
    {
        MDS_LOG_NOTE("CMaxLikelihoodByEM::execute(): Initial MDL = " << dMDL);
    }
...
/*        double dDelta = getAbs(dNewValue / dLogLikelihood - 1.0);
        ...
        if( this->checkOption(MaxLikelihoodByEM::LOGGING_ENABLED) )
        {
            MDS_LOG_NOTE("CMaxLikelihoodByEM::execute(): MDL = " << dNewMDL);
        }
...
bool CMaxLikelihoodByEM<C,N>::initFirstComponent(const tContainer& Input)
{
    ...
    Mean *= mds::CScalar<double>(dInvNum);
    ...
    // Mean
    Cov *= mds::CScalar<double>(dInvNum);
...
void CMaxLikelihoodByEM<C,N>::processMStep(const tContainer& Input)
{
    ...
    for( tSize i = 0; i < getNumOfComponents(); ++i )
    {
        ...
        NewMean *= mds::CScalar<double>(dInvPSum);
        NewCov *= mds::CScalar<double>(dInvPSum);
