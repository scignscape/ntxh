 ...
 * Date:    2004/05/03                          \n
 *
 * $Id: mdsLucasKanade.hxx 2100 2012-02-17 07:36:55Z spanel $
 *
 * Description:
...
//=============================================================================
/*
 * Implementation of the mds::image::COpticalFlow class template.
 */
...
{
    MDS_ASSERT(m_WindowSize > 0 && (m_WindowSize % 2) == 1);

    // Allocate auxiliary matrixes
    m_Ix.create(m_WindowSize, m_WindowSize);
    m_Iy.create(m_WindowSize, m_WindowSize);
    m_It.create(m_WindowSize, m_WindowSize);
}
...
bool COpticalFlow<I>::compute(const tPyramid& First, const tPyramid& Second)
{
    ...
    MDS_CHECK(First.getLevels() == Second.getLevels(), return false);
    ...
    mds::math::CDMatrix2x2 A, Ai;

    // Vector B and vector of eigen values of the matrix A
    mds::math::CDVector2 B, Eigens;
    ...
    for( tSize Level = First.getLevels() - 1; Level >= 0; --Level )
    {
        ...
        for( py = 0; py < Size; ++py )
        {
            ...
            for( px = 0; px < Size; ++px )
            {
                ...
                dMaxIx = mds::math::getMax(dMaxIx, m_Ix(py, px) * m_Ix(py, px));
                dMaxIy = mds::math::getMax(dMaxIy, m_Iy(py, px) * m_Iy(py, px));
                Point.x() += 1.0f;
            }
            Point.y() += 1.0f;
        }
...
//        MDS_LOG_NOTE("Eigen values: " << Eigens(0) << ", " << Eigens(1));

        // Invert the A matrix
        double dDet = A(0, 0) * A(1, 1) - A(0, 1) * A(1, 0);
//        if ( mds::math::getAbs(dDet) > 0.000001 )
        if ( mds::math::getAbs(dDet) > 0.0001 )
        {
...
/*            else if( Eigens(0) >= optflow::EIG_TSH )
            ...
            {
/*                MDS_LOG_NOTE("Point (" << tSize(PointFirst.x() + m_Motion.x())
...
{
    // Initial position
    tSize x1 = mds::math::round2Int(m_Point.x());
    tSize y1 = mds::math::round2Int(m_Point.y());

    // Final position
    tSize x2 = mds::math::round2Int(m_Point.x() + m_Motion.x());
    tSize y2 = mds::math::round2Int(m_Point.y() + m_Motion.y());

    // Check the line position
    MDS_CHECK(Image.checkPosition(x1, y1) && Image.checkPosition(x2, y2), return false);
