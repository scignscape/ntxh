 ...
 * Date:    2008/08/29                          \n
 *
 * $Id: mdsWeightedEM.hxx 2101 2012-02-17 08:20:21Z spanel $
 *
 * Description:
...
CWeightedEM<C,N>::CWeightedEM(int Flags)
    ...
    , m_dMinAddChange(mds::math::MaxLikelihoodByEM::MIN_ADD_CHANGE)
{
...
//    double dMDL = -DBL_MAX;
    ...
    if( this->checkOption(mds::math::MaxLikelihoodByEM::LOGGING_ENABLED) )
    {
        MDS_LOG_NOTE("CWeightedEM::execute(): Initial MDL = " << dMDL);
    }
    ...
    {
        // Compute the EM
        double dNewValue = iterateEM(Input, mds::math::MaxLikelihoodByEM::MIN_CHANGE, NumOfRealSamples);
...
//        double dNewMDL = computeMDL(dNewValue, NumOfSamples);
        ...
        if( this->checkOption(mds::math::MaxLikelihoodByEM::LOGGING_ENABLED) )
        {
            MDS_LOG_NOTE("CWeightedEM::execute(): MDL = " << dNewMDL);
        }
...
bool CWeightedEM<C,N>::execute(const tContainer& Input, tSize NumOfClasses)
{
    ...
    {
        // Training
        iterateEM(Input, mds::math::MaxLikelihoodByEM::MIN_CHANGE, NumOfRealSamples);
...
void CWeightedEM<C,N>::initComponentMean(tSize i, const tContainer& Input)
{
    ...
    tSize First = mds::math::round2Int(m_Uniform.random(0, NumOfSamples -1 ));
    tSize Size = mds::math::getMin(mds::math::round2Int(m_Uniform.random(MIN_NUM_OF_SAMPLES, MAX_NUM_OF_SAMPLES)), NumOfSamples - First);
...
void CWeightedEM<C,N>::initComponentCov(tSize i)
{
    ...
    for( tSize j = 0; j < getNumOfComponents(); ++j )
    {
        if( j != i )
        {
            ...
            double dValue = mds::math::getProd<double>(Temp, Temp);
            if( dValue < dMin )
            {
                dMin = dValue;
            }
...
{
    MDS_CHECK(getNumOfComponents() == 1 && m_Maps.getNumOfCols() > 0, return false);
...
/*    tCov AuxCov = m_Components[0].getCov();
    for( tSize k = 0; k < tComponent::NUM_OF_DIMENSIONS; ++k )
    {
        ...
        {
            AuxCov(k) = mds::math::getMax(AuxCov(k), m_Components[l].getCov().get(k));
        }
...
double CWeightedEM<C,N>::computeLogLikelihood(const tContainer& Input)
{
    ...
    return Result.get(mds::math::LogNum::LOG_VALUE);
}
...
{
    MDS_CHECK(i >= 0 && i < getNumOfComponents(), return false);
    MDS_CHECK(j >= 0 && j < getNumOfComponents(), return false);
...
//    double dMaxTrace = mds::math::getMax(mds::math::getSum<double>(Cov1), mds::math::getSum<double>(Cov2));
    double dMaxTrace = 0.5 * mds::math::getSum<double>(Cov1) + 0.5 * mds::math::getSum<double>(Cov2);
    double dMax = c * std::sqrt(dMaxTrace);

    tMean M = Mean1;
    M -= Mean2;
    double dValue = std::sqrt(mds::math::getSumOfSquares<double>(M));

    if( this->checkOption(mds::math::MaxLikelihoodByEM::LOGGING_ENABLED) )
    {
        MDS_LOG_NOTE("CWeightedEM::areSeparated(): " << dValue << " >= " << dMax);
    }
...
{
    MDS_ASSERT(getNumOfComponents() > 0);
    ...
    double dMaxValue = mds::math::getSum<double>(m_Components[0].getCov());

    tSize Max = 0;
    for( tSize i = 1; i < getNumOfComponents(); ++i )
    {
        double dValue = mds::math::getSum<double>(m_Components[i].getCov());
        if( dValue > dMaxValue )
        {
            Max = i;
            dMaxValue = dValue;
...
{
    MDS_ASSERT(Index < getNumOfComponents() && NewIndex < getNumOfComponents());
    ...
    // New values
    double dNewMean = mds::math::MaxLikelihoodByEM::SPLITTING_COEFF * std::sqrt(dMaxValue);
    ...
    for( tSize i = 0; i < getNumOfComponents(); ++i )
    {
        for( tSize j = i + 1; j < getNumOfComponents(); ++j )
        {
            ...
            if( !areSeparated(i, j, iC) )
            {
                ...
                if( this->checkOption(mds::math::MaxLikelihoodByEM::LOGGING_ENABLED) )
                {
                    MDS_LOG_NOTE("CWeightedEM::checkForSimilarComponents():");
                    MDS_LOG_NOTE("  Splitting " << k << " component...");
                }
...
//                NewCov(m) += Temp(m) * Temp(m) * m_Maps(i, j);
        ...
        NewMean *= mds::CScalard(dInvPSum);
        NewCov *= mds::CScalard(dInvPSum);
...
//    static const double INITIAL_NOISE_COEFF = 1.5;
    ...
    for( tSize j = 0; j < mds::math::MaxLikelihoodByEM::MAX_ITERS2; ++j )
    {
        if( this->checkOption(mds::math::MaxLikelihoodByEM::ADD_RAND_NOISE) )
        {
            // Add random noise to every mixture component
            addRandomNoise(dRandNoise);

            // Modify the random noise coefficient
            dRandNoise *= mds::math::MaxLikelihoodByEM::RAND_NOISE_COEFF;
        }
        ...
        for( tSize i = 0; i < mds::math::MaxLikelihoodByEM::MAX_ITERS; ++i )
        {
            // E-step
            processEStep(Input);
            ...
            if( this->checkOption(mds::math::MaxLikelihoodByEM::LOGGING_ENABLED) )
            {
                MDS_LOG_NOTE("CWeightedEM::iterateEM()");
//            MDS_LOG_NOTE("CWeightedEM::iterateEM()");
