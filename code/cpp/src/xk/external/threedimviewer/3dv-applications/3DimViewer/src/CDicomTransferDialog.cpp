...
#else
 bool //?__stdcall
   threadSafeProgressFn(int val, int total, void* customData)
#endif
{
    if (NULL!=customData)
    {
        ...
        *statusDetail = NULL;
        ...
        if ((imageDataSet != NULL) && (*imageDataSet != NULL) && !cbdata->pRecv->opt_bitPreserving)
        {
            ...
            if ((rsp->DimseStatus == STATUS_Success))
            {
                ...
                if (false) //? this does not match the sig
                    //? !DU_findSOPClassAndInstanceInDataSet(*imageDataSet, sopClass, sopInstance,  cbdata->pRecv->opt_correctUIDPadding))
                {
