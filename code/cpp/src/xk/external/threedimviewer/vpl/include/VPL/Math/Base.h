...
#ifndef VPL_MathBase_H
#define VPL_MathBase_H
...
template <>
inline void swap2(unsigned& a, unsigned& b)
{
    a ^= b;
    b ^= a;
    a ^= b;
...
template <>
inline void swap2(long& a, long& b)
{
    a ^= b;
    b ^= a;
    a ^= b;
...
template <>
inline void swap2(unsigned long& a, unsigned long& b)
{
    a ^= b;
    b ^= a;
    a ^= b;
...
{
    VPL_STATIC_ASSERT(CTypeTraits<T>::isFloat, Bad_template_parameter_type);
...
{
    VPL_STATIC_ASSERT(CTypeTraits<T>::isIntegral, Bad_template_parameter_type);
...
//! \brief Maps the value \p x from the range A to the range B.
//! \param[in] x Value to transform in the range A.
//! \param[in] minA Minimum A.
//! \param[in] maxA Maximum A.
//! \param[in] minB Minimum B.
//! \param[in] maxB Maximum B.
//! \return The transformed value in the B range.
template <typename T>
T linearMapping(const T x,
                const T minA, const T maxA,
