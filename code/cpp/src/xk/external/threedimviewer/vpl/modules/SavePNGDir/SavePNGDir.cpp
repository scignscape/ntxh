...
bool CSavePNGDir::saveImageToDir(ImageType& Image)
{
    ...
    // Save PNG image
    bool bSaved = vpl::img::savePNG(Image, FileChannel);
    ...
    if( !bSaved )
    {
        VPL_LOG_ERROR('<' << this->getFilename() << "> Failed to write the output PNG image: " << Stream.str() << std::endl);
        return false;
    }
...
void CSavePNGDir::writeExtendedUsage(std::ostream& Stream)
{
