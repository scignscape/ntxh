	...
	double*&			bvec)
{
    ...
    //=================================================================================
    void DrawContoursAroundSegments( unsigned int*& ubuff, 
				     int*& labels, 
				     const int&	width, 
				     const int&	height, 
				     const unsigned int&  ) {
...
//    const bool hexgrid = false;
    ...
    //===========================================================================
    void SaveSuperpixelLabels(  int*& labels, 
			        int& width, 
			        int& height, 
...
//	const int dy8[8] = { 0, -1, -1, -1, 0, 1, 1,  1};
    ...
    //===========================================================================
    void DoSuperpixelSegmentation_ForGivenSuperpixelSize( const unsigned int* ubuff, 
							  const int width, 
							  const int height, 
							  int*& klabels, 
							  int& numlabels, 
...
void
convertToARGB( const Imc2duc & ims, unsigned int * output) {
   for (int y = 0; y < ims.Height(); y++) {
      for (int x = 0; x < ims.Width(); x++) {
	 output[y * ims.Width() + x] = ims(0, y, x) << 16
	    | ims(1, y, x) << 8  
	    | ims(2, y, x);
      }
   }
...
void
convertToPandore( int * input, Imc2duc & imd ) { 
   for (int y = 0; y < imd.Height(); y++) {
      for (int x = 0; x < imd.Width(); x++) {
	 imd(0, y, x) = (input[y * imd.Width() + x] >> 16) & 0xff;
	 imd(1, y, x) = (input[y * imd.Width() + x] >> 8) & 0xff;
	 imd(2, y, x) = (input[y * imd.Width() + x]) & 0xff;
      }
   }
}

Errc
PSimpleLinearIterativeClusteing( const Imc2duc &ims, Reg2d &rgd, int k, float m ) {
...
//   ReadImage(pbuff, width, height);//YOUR own function to read an image into the ARGB format
   ...
   const string savepath = "yourpathname";
   ...
   //----------------------------------
   Imc2duc imd(ims.Props());
   convertToPandore(klabels, imd);
   ...
   return PLabeling(imd, rgd, 8);
}

#ifdef MAIN
#define	USAGE	"usage: %s k m  [-m mask] [im_in|-] [rg_out|-]"
#define	PARC	2
#define	FINC	1
#define	FOUTC	1
#define	MASK	3
...
int main( int argc, char *argv[] ) {
   ...
   if (objs[0]->Type() == Po_Imc2duc) {
      ...
      result = PSimpleLinearIterativeClusteing(*ims, *rgd, atoi(parv[0]), (float)atof(parv[1]));
      goto end;
   } 
   {
