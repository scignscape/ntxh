...
bool dtkComposerWriter::write(const QString& fileName, Type type)
{
    if (!d->scene)
        return false;

    return writeNode(d->scene->root(), fileName, type);
}

bool dtkComposerWriter::writeNode(dtkComposerSceneNodeComposite *node, const QString& fileName, Type type)
{
    if (!d->scene)
        return false;
...
QDomElement dtkComposerWriter::writeNode(dtkComposerSceneNode *node, QDomElement& element, QDomDocument& document)
{
    ...
    if (dtkComposerSceneNodeComposite *composite = dynamic_cast<dtkComposerSceneNodeComposite *>(node)) {
        ...
        if(composite->embedded() &&  d->id !=1 ){
            // if id = 1 it means it's a spawn node embedded in a control, don't set it as block
