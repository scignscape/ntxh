...
// //  the code adapting udpipe functions for Qt data types is:
//
...
//  QStringList& files, QString model_file);

int main_with_qmap(int argc, char* argv[], QMap<QString, QString>& qoptions, QStringList& files,
  QString model_file, QVector<sentence*>* sv)
{
 options::map options;
 QMapIterator<QString, QString> it(qoptions);
 while(it.hasNext())
 {
  it.next();
  options[it.key().toStdString()] = it.value().toStdString();
 }
 //QStringList files;
 return main_with_options(argc, argv, options, files, model_file, sv);
}

int main_with_qmap(QMap<QString, QString>& qoptions, QStringList& files,
  QString model_file, QVector<sentence*>* sv)
{
 options::map options;
 QMapIterator<QString, QString> it(qoptions);
 ...
 //QStringList files;
 return main_with_options(options, files, model_file, sv);
}
               ...
               QStringList& infiles, QString outfile)
{
 ...
 return main_with_qmap(qm, infiles, model_file);
}
               ...
               QStringList& infiles, QString outfile)
{
 ...
 return main_with_qmap(argc, argv, qm, infiles, model_file);
}


int main_detokenize(int argc, char* argv[],
                    QStringList& infiles, QString outfile, QVector<sentence*>* sv)
{
 iostreams_init();
 QMap<QString, QString> qm;
 qm["outfile"] = outfile;
 ...
 //QStringList files{"C:/udpipe/eng/t2.txt"};
 return main_with_qmap(argc, argv, qm, infiles, QString(), sv);
}

int main_detokenize(QStringList& infiles, QString outfile, QVector<sentence*>* sv)
{
 return main_detokenize(0, nullptr, infiles, outfile, sv);
}
...
int mainc(int argc, char* argv[]) {
  ...
  QStringList files;
  return main_with_options(argc, argv, options, files, QString::fromStdString(argv[1]));
}

void main_detokenize(options::map& options,
  QStringList& files, QString fn, QVector<sentence*>* sv)
{
 main_detokenize(0, nullptr, options, files, fn, sv);
}
...
// argc = 0;
 ...
 if(!fn.isEmpty())
 {
  ifstream raw_text_file(fn.toStdString());
  if (!raw_text_file.is_open()) runtime_failure("Cannot load raw text from file '" << fn.toStdString() << "'.");
 ...
 {
  _argv[1] = (char*) fn.toStdString().c_str();
  for(int i = 2; i < sz; ++i)
  {
   strv[i - 2] = files[i - 2].toStdString();
   _argv[i] = (char*) strv[i - 2].c_str();
  }
 }



 process_args_with_output_template(2, sz, _argv, options["outfile"], [&detokenizer, sv](istream& is, ostream& os, string, string) {
   unique_ptr<input_format> conllu_input(input_format::new_conllu_input_format());
...
void main_run(int argc, char* argv[], options::map& options,
  QStringList& files, QString model_file)
{
 ...
 if (options.count("tokenizer") || options.count("tokenize") ||
   ...
   model.reset(model::load(model_file.toStdString().c_str()));
   if (!model) runtime_failure("Cannot load UDPipe model '" << model_file.toStdString().c_str() << "'!");
   cerr << "done." << endl;
 }
 ...
 if (options.count("accuracy")) {
   ...
   {
    _argv[1] = (char*) model_file.toStdString().c_str();
    for(int i = 2; i < sz; ++i)
    {
     sv[i - 2] = files[i - 2].toStdString();
     _argv[i] = (char*) sv[i - 2].c_str();
    }
   }
   ...
   // Process the data
   process_args_with_output_template(binary_input, binary_output,
     2, sz, _argv, options["outfile"], [&pipeline](istream& is, ostream& os, string is_name, string) {
     string error;
...
int main_with_options(int argc, char* argv[], options::map& options,
  QStringList& files, QString model_file, QVector<sentence*>* sv)
{
  // TRAINING
  ...
  {
   main_detokenize(argc, argv, options, files, model_file, sv);
  }
  else
  // RUN
  {
   main_run(argc, argv, options, files, model_file);
  }

  return 0;
}

void main_run(options::map& options,
  QStringList& files, QString model_file)
{
 main_run(0, nullptr, options, files, model_file);
}
  ...
  QStringList& files, QString model_file, QVector<sentence*>* sv)
{
 if (options.count("detokenize"))
 {
  main_detokenize(options, files, model_file, sv);
 }
 else
 // RUN
 {
  main_run(options, files, model_file);
 }
