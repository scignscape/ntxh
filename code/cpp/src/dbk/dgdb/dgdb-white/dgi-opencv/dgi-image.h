...
struct Demo_Transform_Group
{
 DGI_Image* src;
 DGI_Image* blurred;
 DGI_Image* draw_contours;
 DGI_Image* circles;
 DGI_Image* approx_poly;
 DGI_Image* hull_poly;
 DGI_Image* hull_poly_color_mean;

 Demo_Transform_Group(DGI_Image* src)
   :  src(src),
      blurred(nullptr),
      draw_contours(nullptr),
      circles(nullptr),
      approx_poly(nullptr),
      hull_poly(nullptr),
      hull_poly_color_mean(nullptr)
 {

 }
};

class DGI_Image : QVector<DGI_Contour*>
{
 cv::Mat matrix_;
 QString path_;

 DGI_Image* ref_parent_image_;
 DGI_Image* parent_image_;
...
public:
 ...
 DGI_Image(cv::Mat matrix);
 DGI_Image(QString path);

 ACCESSORS__CONST_RGET(cv::Mat ,matrix)

 ACCESSORS(QString ,path)

 ACCESSORS(DGI_Image* ,ref_parent_image)
