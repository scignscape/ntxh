...
DGI_Contour::DGI_Contour(std::vector<cv::Point>& c)
  :  points_(c.begin(), c.end()),
     area_(0), rotated_rectangle_aspect_ratio_(0),
     convexity_(0), inner_to_encosing_circle_ratio_(0),
     center_to_inner_circle_norm_(0),
     center_to_enclosing_circle_norm_(0),
     inner_to_enclosing_circle_norm_(0),
     color_averge_blue_(0), color_averge_green_(0),
     color_averge_red_(0),
     nesting_depth_(0), child_contours_count_(0)
{

}

int _count_child_contours(int i, const std::vector<cv::Vec4i>& data, QMap<int, int>& prior_count)
{
 if(data[i][2] == -1)
 {
  prior_count[i] = 0;
 ...
 {
  ++result;
  result += _count_child_contours(j, data, prior_count);
  j = data[j][0];
 }

 prior_count[i] = result;

 return result;
}

int DGI_Contour::count_child_contours(int i, const std::vector<cv::Vec4i>* data)
{
 static QMap<int, int> prior_count;
 ...
 if(prior_count.contains(i))
   return prior_count[i];

 return _count_child_contours(i, *data, prior_count);
}


int _get_contour_depth(int i, const std::vector<cv::Vec4i>& data,
  QMap<int, quint8>& prior_calc)
{
 if(data[i][3] == -1)
 {
  prior_calc[i] = 0;
 ...
 int j = data[i][3];
 ++result;
 result += _get_contour_depth(j, data, prior_calc);
...
int DGI_Contour::get_contour_depth(int i,
  const std::vector<cv::Vec4i>* data)
{
 static QMap<int, quint8> prior_calc;
 ...
 if(prior_calc.contains(i))
   return prior_calc[i];

 return _get_contour_depth(i, *data, prior_calc);
}
