...
#ifndef DGI_CONTOUR__H
#define DGI_CONTOUR__H
...
class DGI_Contour 
{
 ...
 double area_;
 double rotated_rectangle_aspect_ratio_;
 double convexity_;
 double inner_to_encosing_circle_ratio_;
 double center_to_inner_circle_norm_;
 double center_to_enclosing_circle_norm_;
 double inner_to_enclosing_circle_norm_;

 double color_averge_blue_;
 double color_averge_green_;
 double color_averge_red_;


 quint8 nesting_depth_;
 int child_contours_count_;

 cv::Vec4i hierarchy_info_;

public:

 DGI_Contour(std::vector<cv::Point>& c);

 ACCESSORS(double ,area)
 ACCESSORS(double ,rotated_rectangle_aspect_ratio)
 ACCESSORS(double ,convexity)

 ACCESSORS(double ,inner_to_encosing_circle_ratio)

 ACCESSORS(double ,center_to_inner_circle_norm)
 ACCESSORS(double ,center_to_enclosing_circle_norm)
 ACCESSORS(double ,inner_to_enclosing_circle_norm)

 ACCESSORS(double ,color_averge_blue)
 ACCESSORS(double ,color_averge_green)
 ACCESSORS(double ,color_averge_red)


 ACCESSORS(quint8 ,nesting_depth)
 ACCESSORS(int ,child_contours_count)



 ACCESSORS(cv::Vec4i ,hierarchy_info)

 static int count_child_contours(int i = 0,
   const std::vector<cv::Vec4i>* data = nullptr);

 static int get_contour_depth(int i = 0,
   const std::vector<cv::Vec4i>* data = nullptr);
