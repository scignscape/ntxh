...
public:
 ...
 T get(const string& key)
	{
  QString* alt = nullptr;
  boost::any result = getValueByKey(key, (void**) &alt);
  if(alt)
  {
   if(key.front() == '#')
     return boost::any_cast<const T&>(alt->toInt());
   std::string ss = alt->toStdString();
   return boost::any_cast<const T&>(ss);
  }
  return boost::any_cast<const T&>(result);
	}
 ...
 template<typename T>
 void set_suppmental_qmap(T* map_value)
 {
  supplemental_qmap_ = map_value;
 }
...
protected:
 ...
 void* supplemental_qmap_;

 TESTABLE const boost::any& getValueByKey(const string& key, void** alt) const;
