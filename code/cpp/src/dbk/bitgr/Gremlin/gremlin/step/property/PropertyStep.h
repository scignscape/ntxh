...
class PropertyStep: public TraversalStep {
    private:
        std::vector<QString> keys; //duplicates are allowed, per api
        PropertyStepType ps_type;

    public:
        PropertyStep(PropertyStepType type, std::vector<QString> property_keys)
        : TraversalStep(MAP, PROPERTY_STEP) {
            this->keys = property_keys;
            this->ps_type = type;
        }

        std::vector<QString>& get_keys() { return keys; }
