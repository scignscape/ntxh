...
void browngraph_strategy(CPUGraph* bg, QVector<TraversalStep*>& steps);
	...
	for(auto it = steps.begin() + 1; it != steps.end(); ++it) {
		...
		if(current_step->uid == HAS_STEP) {
			...
			GraphStep* graph_step = static_cast<GraphStep*>(current_step);
                        BrownGraphStep* browngraph_step = new BrownGraphStep(false, graph_step->getType(), graph_step->get_element_ids());
                        *it = browngraph_step;
		}
	}
}
