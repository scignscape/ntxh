...
class BitVertex : public Vertex {
	...
	private:
		...
		QString vertex_label;

		// Boolean whether or not it has a label
		bool has_label;

		// The properties
  std::unordered_map<QString, VertexProperty<QVariant>*> my_properties;
	...
	public:
		...
		BitVertex(CPUGraph* graph, uint64_t vid, QString v_label) {
			this->graph = graph;
			this->vertex_id = vid;
			this->has_label = true;
		...
		*/
  virtual QVariant id() {
   return QVariant::fromValue(vertex_id);
		}
		...
		*/
		virtual QString label() {
			return has_label ? QString(vertex_label) : NULL;
		}
  ...
  virtual VertexProperty<QVariant>* property(Cardinality card, QString key, QVariant& value) {
			...
			if(card == SINGLE) {
    this->my_properties[key] = new VertexProperty<QVariant>(SINGLE, key, {value});
			}
			else if(card == SET || card == LIST) { //TODO this is incorrect
				if(old_prop != this->my_properties.end()) {
     std::vector<QVariant> vals = old_prop->second->values();
					vals.push_back(value);
     this->my_properties[key] = new VertexProperty<QVariant>(card, key, vals);
				}
				else {
     this->my_properties[key] = new VertexProperty<QVariant>(card, key, {value});
				}
			}
			else {
