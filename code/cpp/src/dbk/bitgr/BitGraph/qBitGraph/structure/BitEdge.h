...
class BitEdge : public Edge {
	...
	public:
		...
		*/
		BitEdge(CPUGraph* graph, uint64_t id, Vertex* out, Vertex* in, QString label) {
			this->graph = graph;
			this->edge_id = id;
			this->out_vertex = out;
			this->in_vertex = in;
		...
		*/
  virtual QVariant id() { return QVariant::fromValue(this->edge_id); }
		virtual Graph* getGraph() { return static_cast<Graph*>(this->graph); }
		virtual QString label() { return this->edge_label; }
