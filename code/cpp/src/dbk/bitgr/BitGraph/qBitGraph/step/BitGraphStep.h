...
class BitGraphStep: public GraphStep {
    ...
    public:
        BitGraphStep(bool start, GraphStepType gs_type, std::vector<QVariant> element_ids) 
        : GraphStep(gs_type, element_ids) {
            this->start = start;
        }

        virtual void apply(GraphTraversal* trv, TraverserSet& traversers) {
            if(start) traversers.push_back(new Traverser(QVariant())); // add an empty traverser when this is being used as a start step.
            ...
            std::vector<QVariant> element_ids = this->get_element_ids();
            ...
            if(element_ids.empty()) {
                ...
                std::for_each(traversers.begin(), traversers.end(), [&](Traverser* t) {
                    for(int k = 0; k < bg->numVertices(); ++k)
                     new_traversers.push_back(new Traverser(QVariant::fromValue(vertices[k])));
                });
