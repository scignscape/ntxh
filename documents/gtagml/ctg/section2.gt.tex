...
see the \q{parse\_sxp} method in file \textbf{ntxh-udp-sentence.cpp}).\;\<
}
...
\udref{en_gum-ud-train}{GUM_voyage\_merida-20}
\end{sentenceList}
...
\udref{en_gum-ud-train}{GUM_voyage\_socotra-16}
\end{sentenceList}
...
beach as a kind of camping, in effect).\;{\sssg}\footnote{If it seems better to read camping as a \i{noun} \mdash{} the act or phenomenon
...
to a proposition, which could be notated with something like \NNtoProp{}.\;{\sssg}\footnote{A note on notation: I adopt the Haskell convention (referring to the Haskell
...
phrase has a fixed index number.\;{\sssg}\footnote{The subject at position one, for instance;
...
\PropToNYieldsN{}.\;{\sssg}\footnote{Numeric indices take the place of left and right adjacency
...
of particular word-senses, to reason through contrasts in usage.\;{\sssg}\footnote{So, conceiving microclasses similar in spirit to Steven Pinker in
...
{\srsg}\+Microclasses can point out similarities
...
\Gardenfors{} points out, phenomenologists like Jean Petitot.\;{\sssg}\footnote{
...
{\srsg}\+At the same time, Petitot also links with a
...
a large volumes Petitot co-edited.\;{\sssg}\footnote{Petitot's and Barry Smith's formalizing projects were parallel
...
people's experience.\;{\sssg}\footnote{So for instance, Husserl's examination of
...
\udref{en_gum-ud-train}{GUM_voyage\_phoenix-30}
...
(morphosyntactic) context.\;{\sssg}\footnote{Obviously \q{first person} in this setting concerns
...
\q{autobiographical} reading, since (absent some bizarre
...
(\ref{itm:crate}) and (\ref{itm:Sauternes})).\;{\sssg}\footnote{Granted that, perhaps, with a lot of imagination
...
{\srsg}\+Failure to identify these
